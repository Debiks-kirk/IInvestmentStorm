...
const COLLECTION_CARD_TEXT: Record<CardId, string> = {
  luckyTickets: '至多 5 个号码，每号注入 2 金币。', tieCharm: '并列时自动保留排名，多人持有则抵消。', sleeveUpgrade: '消耗一张卡随机升一档。',
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (session.phase === 'finalReceiptHandoff' || session.phase === 'finalReceipt') {
        ...
        try {
        if (session.lottery && !session.lottery.tickets.some(ticket => ticket.playerId === currentPlayer.id && ticket.roundIndex === session.roundIndex && ticket.source !== 'gift') && availableLotteryNumbers(session.lottery, session.players.length).length && botWantsLottery(session.id, session.roundIndex, currentActor, session.lottery.openingPoolUnits, session.players.length, session.roundIndex === session.settings.rounds - 1)) {
