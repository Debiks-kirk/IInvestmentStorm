...
import { IDENTITY_DEFINITIONS, createPlayerIdentity, dealIdentityChoices, getIdentityDefinition, identityValidationErrors, randomLobbyistTask, routeCardAwards, taskLabel } from './game/identities'
...
function PrivateTurn({ session, onSubmit, onAcknowledgeGrant, onAcknowledgeNotice }: { session: GameSession; onSubmit: (turn: RoundTurn) => void; onAcknowledgeGrant: (playerId: string) => void; onAcknowledgeNotice: (noticeId: string) => void }) {
  ...
  const reverserAffordable = player.balanceUnits >= bidUnits + Math.round(reverserCost * 2)
