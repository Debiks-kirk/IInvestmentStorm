              ...
              <><button className="advanced-backdrop" aria-label="关闭高级设置" onClick={() => setAdvanced(false)} /><section className="advanced-settings" role="dialog" aria-modal="true" aria-labelledby="advanced-settings-title"><header className="advanced-settings__head"><div><p className="eyebrow">可随时保存到配置</p><h2 id="advanced-settings-title">高级规则</h2><small>调整后会立即反映在本局摘要与保存的配置中。</small></div><button className="icon-button" aria-label="关闭高级设置" onClick={() => setAdvanced(false)}>×</button></header><div className="advanced-settings__body">
                ...
                <div className="identity-setting-group">
                  ...
                  {settings.identitySettings.enabled && <><strong>启用身份</strong><div className="identity-toggle-grid">{IDENTITY_DEFINITIONS.map((identity) => <label key={identity.id}><input type="checkbox" checked={!settings.identitySettings.disabledIdentityIds.includes(identity.id)} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, disabledIdentityIds: event.target.checked ? settings.identitySettings.disabledIdentityIds.filter((id) => id !== identity.id) : [...settings.identitySettings.disabledIdentityIds, identity.id] } })} /><span>{identity.symbol} {identity.name}</span></label>)}</div><div className="identity-settings-fields">
                    ...
                    <label>预言家观身份费用<input type="number" min="0" max="20" step="0.5" value={settings.identitySettings.prophetDivinationCoins} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, prophetDivinationCoins: Number(event.target.value) } })} /></label>
...
function PrivateTurn({ session, onSubmit, onAcknowledgeGrant, onAcknowledgeNotice, onStartPrizeReroll, onChoosePrizeReroll, onStartMerchantOffer, onChooseMerchantOffer, onUseProphetDivination, onResolveFateCoin, onArmDeadline }: { session: GameSession; onSubmit: (turn: RoundTurn, timedOut?: boolean) => void; onAcknowledgeGrant: (playerId: string) => void; onAcknowledgeNotice: (noticeId: string) => void; onStartPrizeReroll: (playerId: string) => void; onChoosePrizeReroll: (itemId: string) => void; onStartMerchantOffer: (playerId: string) => void; onChooseMerchantOffer: (cardId: CardId) => void; onUseProphetDivination: (playerId: string, mode: ProphetDivination['mode'], targetPlayerId?: string, identityId?: IdentityId) => boolean; onResolveFateCoin: (playerId: string, result: 'heads' | 'tails') => CardUse | null; onArmDeadline: () => void }) {
  ...
  const prophetUses = session.prophetDivinations.filter((entry) => entry.playerId === player.id).length
  ...
  const submitCurrentChoice = (timedOut = false) => {
    ...
    const resolvedIdentityAction = identityAction ?? (pendingMerchantOffer?.chosenCardId ? { type: 'merchantAuction' as const } : currentProphetDivination ? { type: 'prophetDivination' as const, divinationId: currentProphetDivination.id } : undefined)
    ...
    <section className="private-turn">
      ...
      <section className="identity-skills panel">
        ...
        {!identity ? <div className="identity-skill-placeholder"><span>◎</span><div><strong>本局未启用身份系统</strong><small>下局可在高级设置中开启。</small></div></div> : <div className="identity-live">
          ...
          {identity.id === 'prophet' && <div className="prophet-skill"><p><strong>天机推演</strong>是主动技能：每回合至多一次，本局还可使用 <b className="status-token status-token--count">{Math.max(0, session.settings.identitySettings.prophetDivinationLimit - prophetUses)} 次</b>。<b className="status-token status-token--free">观财、观星免费</b>；只有观身份需要 <b className="status-token status-token--cost">{session.settings.identitySettings.prophetDivinationCoins} 金币</b>。</p>{currentProphetDivination ? <ProphetResult divination={currentProphetDivination} session={session} /> : <button className="button button--prophet" disabled={prophetUses >= session.settings.identitySettings.prophetDivinationLimit} onClick={() => setProphetDialog('menu')}>{prophetUses >= session.settings.identitySettings.prophetDivinationLimit ? '本局推演次数已用完' : '发动天机推演'}</button>}</div>}
          ...
          {identity.id === 'reverser' && <><p>本局还可发动 <b className="status-token status-token--count">{Math.max(0, activeSkillLimit - activeSkillUses)} 次</b>。花费 {reverserCost} 金币，倒转本轮获奖区内的所有名次；最后两轮费用翻倍。若同时使用“逆转排名”道具卡，两次逆转会抵消。</p><button className={cx('button', identityAction?.type === 'reverserInvert' && 'button--primary')} disabled={!identityAction && (!reverserAffordable || activeSkillLimitReached)} onClick={() => identityAction?.type === 'reverserInvert' ? setIdentityAction(undefined) : setIdentityConfirming({ type: 'reverserInvert' })}>{identityAction?.type === 'reverserInvert' ? '已安排逆转排名 · 点击撤销' : activeSkillLimitReached ? `本局发动次数已用完（${activeSkillUses}/${activeSkillLimit}）` : reverserAffordable ? `花费 ${reverserCost} 金币发动` : `余额不足，还差 ${formatCoins(reverserShortfallUnits)} 金币`}</button></>}
      ...
      {pendingMerchantOffer && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="confirm-sheet"><p className="eyebrow">道具商人 · 已锁定候选</p><h2>选择下一轮要竞购的道具</h2><p>三张候选已经抽出并保存；选择后不能重抽，另外两张会回到循环池。</p><div className="merchant-offer-list">{pendingMerchantOffer.offeredCardIds.map((cardId) => { const card = getCardDefinition(cardId); return <button key={cardId} onClick={() => onChooseMerchantOffer(cardId)}><span>{card.symbol}</span><strong>{card.name} <CardRarityTag cardId={cardId} /></strong><small>{card.description}</small></button> })}</div></section></div>}
      ...
      {prophetDialog === 'menu' && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="target-picker-sheet prophet-picker"><p className="eyebrow">预言家 · 天机推演</p><h2>选择一种推演</h2><p>观财和观星立即免费生效；观身份会支付费用并锁定本轮猜测。</p><div className="prophet-option-grid"><button onClick={() => { if (onUseProphetDivination(player.id, 'wealth')) setProphetDialog(null) }}><span>◒</span><strong>观财</strong><small>免费：读取本轮开始时的最高、最低余额区间，不显示姓名。</small></button><button disabled={prophetFutureItems.length === 0} onClick={() => { if (onUseProphetDivination(player.id, 'stars')) setProphetDialog(null) }}><span>✦</span><strong>观星</strong><small>{prophetFutureItems.length > 0 ? '免费：查看未来至多两轮的预设拍品。' : '最后一轮没有未来拍品。'}</small></button><button disabled={prophetShortfallUnits > 0} onClick={() => setProphetDialog('target')}><span>◉</span><strong>观身份</strong><small>{prophetShortfallUnits > 0 ? `余额不足，还差 ${formatCoins(prophetShortfallUnits)} 金币` : `花费 ${session.settings.identitySettings.prophetDivinationCoins} 金币：猜一名玩家的身份；猜对获得一张道具。`}</small></button></div><button className="button button--paper" onClick={() => setProphetDialog(null)}>取消使用</button></section></div>}
...
function Game({ session, setSession, onExit, onNewGame, onRematch, onRevenge }: { session: GameSession; setSession: (session: GameSession) => void; onExit: () => void; onNewGame: () => void; onRematch: () => void; onRevenge: () => void }) {
  ...
  const useProphetDivination = (playerId: string, mode: ProphetDivination['mode'], targetPlayerId?: string, identityId?: IdentityId): boolean => {
    ...
    const alreadyUsedIdentityThisRound = ownDivinations.some((entry) => entry.roundIndex === session.roundIndex && entry.mode === 'identity')
    const mainUses = ownDivinations.filter((entry) => entry.mode !== 'identity').length
    if (session.phase !== 'privateTurn' || player?.id !== playerId || player.identity?.id !== 'prophet' || (mode === 'identity' ? alreadyUsedIdentityThisRound : alreadyUsedMainThisRound) || (mode !== 'identity' && mainUses >= session.settings.identitySettings.prophetDivinationLimit)) return false
    ...
    const notices = mode === 'identity' && divination.identityGuess ? [...session.pendingIdentityNotices, {
      id: `prophet-identity-feedback-${divination.id}`,
      playerId,
      title: divination.identityGuess.correct ? '观身份猜对了' : '观身份猜错了',
      detail: divination.identityGuess.correct
        ? `你猜对了 ${playerName(session.players, divination.identityGuess.targetPlayerId)} 的身份：${getIdentityDefinition(divination.identityGuess.identityId).name}。${divination.identityGuess.rewardCardId ? `获得了 ${getCardDefinition(divination.identityGuess.rewardCardId).name}。` : '但循环卡池暂时没有可发道具。'}`
        : `这次身份猜测不正确。已支付的 ${formatCoins(costUnits)} 金币不会退还，但不会再扣额外罚款。`,
    }] : session.pendingIdentityNotices
    patch({ players, cardDeck, prophetDivinations: [...session.prophetDivinations, divination], pendingIdentityNotices: notices })
    return true
  }
