              ...
              <><button className="advanced-backdrop" aria-label="关闭高级设置" onClick={() => setAdvanced(false)} /><section className="advanced-settings" role="dialog" aria-modal="true" aria-labelledby="advanced-settings-title"><header className="advanced-settings__head"><div><p className="eyebrow">可随时保存到配置</p><h2 id="advanced-settings-title">高级规则</h2><small>调整后会立即反映在本局摘要与保存的配置中。</small></div><button className="icon-button" aria-label="关闭高级设置" onClick={() => setAdvanced(false)}>×</button></header><div className="advanced-settings__body">
                ...
                <div className="identity-setting-group">
                  ...
                  {settings.identitySettings.enabled && <><strong>启用身份</strong><div className="identity-toggle-grid">{IDENTITY_DEFINITIONS.map((identity) => <label key={identity.id}><input type="checkbox" checked={!settings.identitySettings.disabledIdentityIds.includes(identity.id)} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, disabledIdentityIds: event.target.checked ? settings.identitySettings.disabledIdentityIds.filter((id) => id !== identity.id) : [...settings.identitySettings.disabledIdentityIds, identity.id] } })} /><span>{identity.symbol} {identity.name}</span></label>)}</div><div className="identity-settings-fields">
                    ...
                    <label>小偷行动次数<input type="number" min="1" max="12" step="1" value={settings.identitySettings.thiefActivationLimit} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, thiefActivationLimit: Number(event.target.value) } })} /></label>
...
function PrivateTurn({ session, onSubmit, onAcknowledgeGrant, onAcknowledgeNotice, onStartPrizeReroll, onChoosePrizeReroll, onStartMerchantOffer, onChooseMerchantOffer, onChooseProphetOffer, onChooseKidnapOffer, onUseProphetDivination, onResolveFateCoin, onArmDeadline }: { session: GameSession; onSubmit: (turn: RoundTurn, timedOut?: boolean) => void; onAcknowledgeGrant: (playerId: string) => void; onAcknowledgeNotice: (noticeId: string) => void; onStartPrizeReroll: (playerId: string) => void; onChoosePrizeReroll: (itemId: string) => void; onStartMerchantOffer: (playerId: string) => void; onChooseMerchantOffer: (cardId: CardId) => void; onChooseProphetOffer: (playerId: string, cardId: CardId) => void; onChooseKidnapOffer: (playerId: string, cardId: CardId) => void; onUseProphetDivination: (playerId: string, mode: ProphetDivination['mode'], targetPlayerId?: string, identityId?: IdentityId) => boolean; onResolveFateCoin: (playerId: string, result: 'heads' | 'tails') => CardUse | null; onArmDeadline: () => void }) {
  ...
  const activeSkillLimit = identity?.id === 'assassin' ? session.settings.identitySettings.kidnapActivationLimit
    : identity?.id === 'thief' ? session.settings.identitySettings.thiefActivationLimit
  ...
  const pendingKidnapOffer = session.pendingKidnapCardOffers.find((offer) => offer.playerId === player.id)
  ...
  const setAuctionBid = (lotId: string, value: number) => {
    ...
    if (!lot || lot.merchantId === player.id) return
    ...
    setAuctionBidByLotId((bids) => ({ ...bids, [lotId]: Math.max(0, Math.min(max, Math.round(value))) }))
  }
  ...
  const operationReady = !activeNotice && !visibleGrant && !pendingKidnapOffer
  useEffect(() => {
  ...
  const submitCurrentChoice = (timedOut = false) => {
    ...
    const auctionBids = auctionLots.map((lot) => ({ lotId: lot.id, bidUnits: lot.merchantId === player.id ? 0 : auctionBidByLotId[lot.id] ?? 0 }))
    onSubmit({ playerId: player.id, bidUnits, predictedPlayerId: prediction, ...(auctionBids.length ? { auctionBids } : {}), ...(allConfirmedCardUses.length > 0 ? { cardUses: allConfirmedCardUses } : {}), ...(resolvedIdentityAction ? { identityAction: resolvedIdentityAction } : {}) }, timedOut)
  }
  return (
    <section className="private-turn">
      ...
      {auctionLots.length > 0 && <section className="round-auctions panel" aria-label="本轮道具竞购">
        <div className="panel-title"><div><p className="eyebrow">仅自己可见</p><h2>本轮道具竞购</h2></div><span>总报价 <CoinValue units={auctionBidTotal} /></span></div>
        <p className="round-auctions__hint">每张卡分别秘密报价。最高唯一正报价者获得卡；未得标不扣费。</p>
      ...
      <section className="identity-skills panel">
        ...
        {!identity ? <div className="identity-skill-placeholder"><span>◎</span><div><strong>本局未启用身份系统</strong><small>下局可在高级设置中开启。</small></div></div> : <div className="identity-live">
          ...
          {identity.id === 'assassin' && <><p>本局还可行动 <b className="status-token status-token--count">{Math.max(0, activeSkillLimit - activeSkillUses)} 次</b>。花费 {session.settings.identitySettings.kidnapActivationCoins} 金币选择一名玩家。若他拿下本轮拍品，费用会报销，拍品会被你抢走；失败则这笔钱不会返还。</p><button className={cx('button', identityAction?.type === 'kidnap' && 'button--primary')} disabled={!identityAction && (!kidnapAffordable || activeSkillLimitReached)} onClick={() => identityAction?.type === 'kidnap' ? setIdentityAction(undefined) : setTargetPicker('kidnap')}>{identityAction?.type === 'kidnap' ? `已盯上 ${playerName(session.players, identityAction.targetPlayerId)} · 点击撤销` : activeSkillLimitReached ? `本局行动次数已用完（${activeSkillUses}/${activeSkillLimit}）` : kidnapAffordable ? `花费 ${session.settings.identitySettings.kidnapActivationCoins} 金币选择目标` : `余额不足，还差 ${formatCoins(kidnapShortfallUnits)} 金币`}</button></>}
          {identity.id === 'collector' && <p>已为 <strong>{categoryConfig(identity.collectorCategory ?? 'leisure').name}</strong> 永久额外计入 1 件固定资产。</p>}
          {identity.id === 'thief' && <><p>本局还可行动 <b className="status-token status-token--count">{Math.max(0, activeSkillLimit - activeSkillUses)} 次</b>。花费 {session.settings.identitySettings.thiefActivationCoins} 金币。所有人提交后，以 {session.settings.identitySettings.thiefSuccessProbability}% 概率偷走别人一张本轮未使用道具；下回合才会收到结果。</p><button className={cx('button', identityAction?.type === 'thiefSteal' && 'button--primary')} disabled={!identityAction && (!thiefAffordable || session.roundIndex >= session.settings.rounds - 1 || activeSkillLimitReached)} onClick={() => identityAction?.type === 'thiefSteal' ? setIdentityAction(undefined) : setIdentityConfirming({ type: 'thiefSteal' })}>{identityAction?.type === 'thiefSteal' ? '已发动偷卡 · 点击撤销' : activeSkillLimitReached ? `本局行动次数已用完（${activeSkillUses}/${activeSkillLimit}）` : session.roundIndex >= session.settings.rounds - 1 ? '最后一轮不可发动' : thiefAffordable ? `花费 ${session.settings.identitySettings.thiefActivationCoins} 金币发动` : `余额不足，还差 ${formatCoins(thiefShortfallUnits)} 金币`}</button></>}
          ...
          {identity.id === 'investor' && <><p>秘密投资一名其他玩家，金额至少 5 金币。目标获奖后，按出资比例分享排名奖励；你的投入会计入其排名金额。</p>{identityAction?.type === 'invest' ? <div className="identity-config"><strong>已投资 {playerName(session.players, identityAction.targetPlayerId)}</strong><input className="range" type="range" min="10" max={Math.max(10, player.balanceUnits - bidUnits - auctionBidTotal)} step="1" value={identityAction.investmentUnits} onChange={(event) => setIdentityAction({ ...identityAction, investmentUnits: Math.max(10, Math.min(player.balanceUnits - bidUnits - auctionBidTotal, Number(event.target.value))) })} /><b>投资 {formatCoins(identityAction.investmentUnits)} 金币</b><button className="button" onClick={() => setIdentityAction(undefined)}>取消投资</button></div> : <button className="button" disabled={player.balanceUnits - bidUnits - auctionBidTotal < 10} onClick={() => setTargetPicker('invest')}>{player.balanceUnits - bidUnits - auctionBidTotal < 10 ? '至少保留 5 金币才能投资' : '选择投资对象'}</button>}</>}
      ...
      {prophetDialog === 'menu' && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="target-picker-sheet prophet-picker"><p className="eyebrow">预言家 · 天机推演</p><h2>选择一种推演</h2><p>本轮在观财和观星中选择一项，均免费；观身份在技能区单独发动。</p><div className="prophet-option-grid"><button onClick={() => { if (onUseProphetDivination(player.id, 'wealth')) setProphetDialog(null) }}><span>◒</span><strong>观财</strong><small>免费：读取本轮开始时的最高、最低余额区间，不显示姓名。</small></button><button disabled={prophetFutureItems.length === 0} onClick={() => { if (onUseProphetDivination(player.id, 'stars')) setProphetDialog(null) }}><span>✦</span><strong>观星</strong><small>{prophetFutureItems.length > 0 ? '免费：查看未来至多两轮的预设拍品。' : '最后一轮没有未来拍品。'}</small></button></div><button className="button button--paper" onClick={() => setProphetDialog(null)}>取消使用</button></section></div>}
...
function Game({ session, setSession, onExit, onNewGame, onRematch, onRevenge }: { session: GameSession; setSession: (session: GameSession) => void; onExit: () => void; onNewGame: () => void; onRematch: () => void; onRevenge: () => void }) {
  ...
  const chooseKidnapOffer = (playerId: string, cardId: CardId) => {
    const player = session.players[session.currentTurnIndex]
    const offer = session.pendingKidnapCardOffers.find((entry) => entry.playerId === playerId)
    if (session.phase !== 'privateTurn' || player?.id !== playerId || !offer || !offer.offeredCardIds.includes(cardId)) return
    const unchosen = offer.offeredCardIds.filter((id) => id !== cardId)
    patch({
      players: session.players.map((entry) => entry.id === playerId ? { ...entry, cardInventory: [...entry.cardInventory, cardId] } : entry),
      cardDeck: shuffle([...session.cardDeck, ...unchosen]),
      pendingKidnapCardOffers: session.pendingKidnapCardOffers.filter((entry) => entry !== offer),
    })
  }
  ...
  const submitTurn = (turn: RoundTurn, botRecord?: { mode: import('./game/types').StrategyMode; reason: string; intel?: string }, timedOut = false) => {
    ...
    if (auctionBids.length !== session.roundAuctions.length || new Set(auctionBids.map((bid) => bid.lotId)).size !== auctionBids.length) return false
    const auctionBidTotal = auctionBids.reduce((total, bid) => total + bid.bidUnits, 0)
    if (auctionBids.some((bid) => !Number.isInteger(bid.bidUnits) || bid.bidUnits < 0 || !session.roundAuctions.some((lot) => lot.id === bid.lotId) || (session.roundAuctions.find((lot) => lot.id === bid.lotId)?.merchantId === turn.playerId && bid.bidUnits !== 0))) return false
    ...
    if (turn.identityAction?.type === 'thiefSteal') {
      ...
      if (currentPlayer.identity?.id !== 'thief' || (currentPlayer.identity.activeSkillUses ?? 0) >= session.settings.identitySettings.thiefActivationLimit || (currentPlayer.identity.thiefSuccesses ?? 0) >= (session.settings.identitySettings.thiefMaxSteals ?? 2) || session.roundIndex >= session.settings.rounds - 1 || turn.bidUnits + auctionBidTotal + costUnits > currentPlayer.balanceUnits) return false
    }
    if (turn.identityAction?.type === 'invest') {
      const action = turn.identityAction
      const targetValid = action.targetPlayerId !== turn.playerId && session.players.some((player) => player.id === action.targetPlayerId)
      if (currentPlayer.identity?.id !== 'investor' || !targetValid || !Number.isInteger(action.investmentUnits) || action.investmentUnits < 10 || turn.bidUnits + auctionBidTotal + action.investmentUnits > currentPlayer.balanceUnits) return false
    }
  ...
  const beginNormalRound = (roundIndex: number, basePlayers: Player[], baseDeck: CardId[], notices = session.pendingIdentityNotices, events = session.identityEvents) => {
    ...
    const kidnapOffers: Array<{ playerId: string; offeredCardIds: CardId[] }> = []
    for (const kidnapper of grants.players.filter((entry) => entry.identity?.id === 'assassin' && entry.identity.pendingKidnapReward)) {
      const offeredCardIds: CardId[] = []
      for (let index = 0; index < 3; index += 1) {
        const draw = drawCard(deck, session.settings.disabledCardIds)
        if (!draw.cardId) break
        deck = draw.cardDeck
        offeredCardIds.push(draw.cardId)
      }
      if (offeredCardIds.length > 0) kidnapOffers.push({ playerId: kidnapper.id, offeredCardIds })
    }
    const grantedPlayers = grants.players.map((entry) => entry.identity?.pendingKidnapReward ? { ...entry, identity: { ...entry.identity, pendingKidnapReward: false } } : entry)
    const pendingAwards = [...grants.pendingCardGrants, ...merchantAwards.map((award) => ({ ...award, announced: false })), ...reverserAwards.map((award) => ({ ...award, announced: false }))]
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (session.phase === 'identityHandoff') {
        ...
        const kidnapOffer = session.pendingKidnapCardOffers.find((offer) => offer.playerId === currentPlayer.id)
        if (kidnapOffer) {
          chooseKidnapOffer(currentPlayer.id, kidnapOffer.offeredCardIds[0])
          return
        }
