import { instantUses, upgradeChoices, useInstantCard } from './game/expansionCards'
...
function relayScheduleLabel(operators: RelayOperator[], method: RelayMethod, rounds: number): string {
  if (!operators.length) return '待添加操作者'
  ...
  return operators.map((operator, index) => {
    ...
    return length ? `${operator.name || `操作者 ${index + 1}`} ${start}${length > 1 ? `–${start + length - 1}` : ''} 轮` : `${operator.name || `操作者 ${index + 1}`} · 暂无回合`
  }).join(' · ')
}
...
function CardRarityTag({ cardId }: { cardId: CardId }) {
...
const COLLECTION_CARD_TEXT: Record<CardId, string> = {
  luckyTickets: '至多 5 个号码（末轮 3 个），每号注入 2 金币。', tieCharm: '并列时自动保留排名，多人持有则抵消。', sleeveUpgrade: '消耗一张卡随机升一档。',
  ...
  return (
    <AppShell>
      ...
      <section className="history-detail">
        ...
        <section className="history-standings panel"><div className="panel-title"><div><p className="eyebrow">最终资产</p><h2>最终排行榜</h2></div><span>现金 + 固定资产</span></div><div>{standings.map((standing) => <article key={standing.player.id} className={cx('history-standing', standing.place === 1 && 'is-first')} style={{ '--player-color': standing.player.color } as React.CSSProperties}><span>{standing.place}</span><div className="history-standing__avatar"><PlayerAvatar player={standing.player} /></div><div><strong>{standing.player.name}</strong><small>{standing.player.items.length ? standing.player.items.map(({ item }) => `${item.emoji}${item.name}`).join(' · ') : '没有拍品收藏'}</small></div><div className="history-standing__assets"><b><CoinValue units={standing.totalAssetUnits} /></b><small>现金 {formatCoins(standing.cashUnits)} · 固定资产 +{formatCoins(standing.fixedAssetUnits)}</small></div></article>)}</div></section>
...
function CustomBotManager({ profiles, onChange, onClose }: { profiles: CustomBotProfile[]; onChange: (profiles: CustomBotProfile[]) => void; onClose: () => void }) {
  ...
  const [draggingPriorityId, setDraggingPriorityId] = useState<IdentityId | null>(null)
  const [dragOverPriorityId, setDragOverPriorityId] = useState<IdentityId | null>(null)
...
function RegisteredPlayerPicker({ value, members, players, unavailableNames, label, onChange, onRegister }: { value: string; members: MemberProfile[]; players: string[]; unavailableNames: string[]; label: string; onChange: (name: string) => void; onRegister: (name: string) => string | null }) {
  ...
  const memberForName = (name: string) => members.find((member) => memberNameKey(member.name) === memberNameKey(name))
  const selectedMember = registered ? memberForName(registered) : undefined
  ...
  const playerOption = (name: string) => <button type="button" role="option" aria-selected={registered === name} disabled={isUnavailable(name)} key={name} onClick={() => selectPlayer(name)}><span>{memberForName(name) ? <AvatarIcon avatar={memberForName(name)!.avatar} /> : name.slice(0,1)}</span><strong>{name}</strong>{isUnavailable(name) ? <em>已入局</em> : registered === name && <em>当前</em>}</button>
...
function Setup({ onBack, onStart, presets, onSavePresets, customBotProfiles, onSaveCustomBotProfiles, registeredPlayers, onSaveRegisteredPlayers, members, onRegisterMember }: { onBack: () => void; onStart: (session: GameSession) => void; presets: GamePreset[]; onSavePresets: (presets: GamePreset[]) => void; customBotProfiles: CustomBotProfile[]; onSaveCustomBotProfiles: (profiles: CustomBotProfile[]) => void; registeredPlayers: string[]; onSaveRegisteredPlayers: (players: string[]) => void; members: MemberProfile[]; onRegisterMember: (name: string) => MemberProfile | null }) {
  ...
  const transferOperator = (source: number, id: string, destination: number, index: number) => {
    setRelaySeats((current) => moveRelayOperator(current, source, id, destination, index))
    setErrors([])
  }
  const { drag, start: startRelayDrag } = useRelayDrag(transferOperator)
  const modePresets = presets.filter((preset) => (preset.mode === 'relay' ? 'relay' : 'standard') === mode)

  return (
    <AppShell>
      <header className="page-header"><button className="icon-button" onClick={onBack} aria-label="返回">←</button><Brand /><span /></header>
      <section className="setup-page setup-page--lobby">
        <div className="section-heading setup-heading"><div><p className="eyebrow">新对局</p><h1>设置玩家</h1></div><p>填好玩家，马上开局。</p></div>
        ...
        <div className="setup-grid">
          <div className="panel setup-roster-panel">
            {mode === 'standard' ? <>
            ...
            {seats.some((seat) => seat.controller.kind === 'bot') && <div className="bot-setup-note"><span>Bot 的性格、难度和策略在开局后保持隐藏。</span><button className="text-button" onClick={() => setCustomBotsOpen(true)}>管理自定义 Bot</button></div>}
              ...
              <p className="relay-setup__hint">长按序号拖动换棒，也可移到其他玩家。</p>
                  ...
                  <section className={cx('relay-seat-card', drag?.drop?.seat === seatIndex && 'is-drop-target')} data-relay-seat={seatIndex} key={seatIndex} style={{ '--player-color': `var(--player-${seatIndex + 1})` } as React.CSSProperties}>
                    ...
                    <ol className={cx('relay-operator-list', drag?.drop?.seat === seatIndex && drag.drop.index === relaySeat.operators.length && 'drop-at-end')}>
                      {relaySeat.operators.map((operator, operatorIndex) => {
                        ...
                        return <li key={operator.id} data-relay-operator={operator.id} className={cx('relay-operator', Boolean(botController) && 'relay-operator--bot', drag?.id === operator.id && 'is-dragging', drag?.drop?.seat === seatIndex && drag.drop.index === operatorIndex && 'drop-before')}>
                          <button type="button" className="relay-operator__order" title="长按拖动；方向键调整位置" aria-label={`移动${operator.name || `操作者 ${operatorIndex + 1}`}，第 ${operatorIndex + 1} 棒`} onContextMenu={(event) => event.preventDefault()} onPointerDown={(event) => startRelayDrag(event, seatIndex, operator.id, operator.name || `操作者 ${operatorIndex + 1}`)} onKeyDown={(event) => {
                            if (!['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) return
                            event.preventDefault()
                            if (event.key === 'ArrowUp' && operatorIndex > 0) transferOperator(seatIndex, operator.id, seatIndex, operatorIndex - 1)
                            if (event.key === 'ArrowDown' && operatorIndex < relaySeat.operators.length - 1) transferOperator(seatIndex, operator.id, seatIndex, operatorIndex + 2)
                            if (event.key === 'ArrowLeft' && seatIndex > 0) transferOperator(seatIndex, operator.id, seatIndex - 1, relaySeats[seatIndex - 1].operators.length)
              ...
              {drag && createPortal(<div className="relay-drag-ghost" style={{ left: drag.x, top: drag.y }}><span>⠿</span>{drag.name}</div>, document.body)}
          ...
          <div className="panel settings-panel">
            <div className="setup-panel-title setup-panel-title--rules"><div><p className="eyebrow">规则</p><h2>本局参数</h2></div></div>
            ...
            {advanced && <SetupRulesModal onClose={() => setAdvanced(false)}>
              <><button className="advanced-backdrop" aria-label="关闭高级设置" onClick={() => setAdvanced(false)} /><section className="advanced-settings" role="dialog" aria-modal="true" aria-labelledby="advanced-settings-title"><header className="advanced-settings__head"><div><p className="eyebrow">可随时保存到配置</p><h2 id="advanced-settings-title">高级规则</h2><small>调整后会立即反映在本局摘要与保存的配置中。</small></div><button className="icon-button" aria-label="关闭高级设置" onClick={() => setAdvanced(false)}>×</button></header><div className="advanced-settings__body">
                ...
                <div className="card-setting-group"><strong>禁用道具卡</strong><small>未勾选的卡会加入本局循环卡池；使用后回池，未使用会留在手中</small><div>{CARD_DEFINITIONS.map((card) => <label key={card.id}><input type="checkbox" checked={!settings.disabledCardIds.includes(card.id)} onChange={(event) => setSettings({ ...settings, disabledCardIds: event.target.checked ? settings.disabledCardIds.filter((id) => id !== card.id) : [...settings.disabledCardIds, card.id] })} /><span><CardIcon id={card.id} /> {card.name} <CardRarityTag cardId={card.id} /></span></label>)}</div></div>
                <div className="identity-setting-group">
                  ...
                  {settings.identitySettings.enabled && <><strong>启用身份</strong><div className="identity-toggle-grid">{IDENTITY_DEFINITIONS.map((identity) => <label key={identity.id}><input type="checkbox" checked={!settings.identitySettings.disabledIdentityIds.includes(identity.id)} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, disabledIdentityIds: event.target.checked ? settings.identitySettings.disabledIdentityIds.filter((id) => id !== identity.id) : [...settings.identitySettings.disabledIdentityIds, identity.id] } })} /><span><IdentityIcon id={identity.id} /> {identity.name}</span></label>)}</div><div className="identity-settings-fields">
                    ...
                    <label>夜行者双影次数<input type="number" min="1" max="12" step="1" value={settings.identitySettings.nightwalkerUseLimit} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, nightwalkerUseLimit: Number(event.target.value) } })} /></label>
                    ...
                    <label>投资者分红倍率<input type="number" min="1" max="3" step="0.05" value={settings.identitySettings.investorDividendMultiplier} onChange={(event) => setSettings({ ...settings, identitySettings: { ...settings.identitySettings, investorDividendMultiplier: Number(event.target.value) } })} /></label>
...
function RoundIntro({ session, onContinue, auto = false }: { session: GameSession; onContinue: () => void; auto?: boolean }) {
  ...
  return (
    <section className="round-intro screen-center">
      ...
      ) : <div className="round-prize-stack"><PrizeCard item={item} /><div className="category-holders panel"><div className="category-bonus-head"><small>{category.name} · 本件固定资产</small><strong>+{itemBonusCoins}</strong></div><div className="category-bonus-track" aria-label={`${category.name} 固定资产加成`}><span className="is-item">1 件 <b>+{itemBonusCoins}</b></span><span>2 件 <b>+{category.tiers[0]}</b></span><span>3 件 <b>+{category.tiers[1]}</b></span><span>4 件 <b>+{category.tiers[2]}</b></span><span>5 件 <b>+{category.tiers[3]}</b></span><span>6+ <b>+{category.additionalUnit}/件</b></span></div><small className="category-holders__label">已有这类拍品</small>{categoryHolders.length === 0 ? <strong>目前无人持有</strong> : <div>{categoryHolders.map(({ player, count }) => <span key={player.id}>{player.name} × {count}</span>)}</div>}</div></div>}
...
function SpectatorControls({ paused, speed, onToggle, onStep, onSpeed, onPanel }: { paused: boolean; speed: number; onToggle: () => void; onStep: () => void; onSpeed: (speed: number) => void; onPanel: () => void }) {
  return <div className="spectator-controls" aria-label="Bot 观战控制"><strong><i>LIVE</i> 观战模式</strong><button onClick={onToggle}>{paused ? '▶ 继续' : 'Ⅱ 暂停'}</button><button onClick={onStep} disabled={!paused}>▷ 单步</button><label><span>速度</span><select aria-label="观战速度" value={speed} onChange={(event) => onSpeed(Number(event.target.value))}><option value={0.5}>0.5×</option><option value={1}>1×</option><option value={2}>2×</option><option value={4}>4×</option></select></label><button className="spectator-controls__panel" onClick={onPanel}>▦ 数据</button></div>
}
...
function SpectatorEventStage({ event, session }: { event: SpectatorEvent; session: GameSession }) {
  ...
  const icon = event.type === 'identityChoice' ? identity ? <IdentityIcon id={identity.id} /> : '◇' : event.type === 'roundResult' ? '✓' : event.type === 'kidnap' ? <IdentityIcon id="assassin" /> : event.type.includes('auction') || event.type === 'assetSale' ? '◈' : '●'
  return <section className={`spectator-event-stage spectator-event-stage--${event.type}`} aria-live="polite" data-testid="spectator-event"><div className="spectator-event-stage__pulse" aria-hidden="true" /><article><div className={cx("spectator-event-stage__icon", typeof icon !== "string" && "game-art-slot")} style={player ? { '--player-color': player.color } as React.CSSProperties : undefined}>{icon}</div><p className="eyebrow">第 {event.roundIndex + 1} 轮 · {event.type === 'identityChoice' ? '身份确认' : event.type === 'roundResult' ? '回合落定' : '行动确认'}</p><h1>{event.summary}</h1>{player && <div className="spectator-event-stage__profile"><span style={{ background: player.color }}><PlayerAvatar player={player} /></span><div><strong>{player.name} · {identity?.name ?? '未选身份'}</strong><small>{operator ? `${operator.name} 操作 · ` : ''}{profile?.name ?? '自定义'} · {controller?.difficulty === 'expert' ? '高手' : controller?.difficulty === 'easy' ? '简单' : '标准'}</small></div></div>}<div className="spectator-event-stage__details">{event.details.map((detail, index) => <span key={`${detail}-${index}`}>{detail}</span>)}</div></article></section>
}

function SpectatorChart({ session, chartKey }: { session: GameSession; chartKey: SpectatorChartKey }) {
  const rows = createSpectatorChart(session, chartKey); const width = 560; const height = 220; const pad = 25
  const maximum = Math.max(2, ...rows.flatMap((row) => Object.values(row.values)))
  const points = (playerId: string) => rows.map((row, index) => `${pad + (rows.length === 1 ? 0 : index * (width - pad * 2) / (rows.length - 1))},${height - pad - ((row.values[playerId] ?? 0) / maximum) * (height - pad * 2)}`).join(' ')
  return <div className="spectator-chart"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="对局走势折线图"><g className="spectator-chart__grid"><line x1={pad} y1={pad} x2={pad} y2={height - pad} /><line x1={pad} y1={height - pad} x2={width - pad} y2={height - pad} /><line x1={pad} y1={height / 2} x2={width - pad} y2={height / 2} /></g>{session.players.map((player) => <polyline key={player.id} points={points(player.id)} fill="none" stroke={player.color} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />)}</svg><div>{session.players.map((player) => <span key={player.id}><i style={{ background: player.color }} />{player.name}</span>)}</div></div>
}

type SpectatorPanelTab = 'battle' | 'data' | 'events'

function SpectatorDashboard({ session, tab, open, selectedPlayerId, eventFilter, onTab, onClose, onSelectPlayer, onFilter }: { session: GameSession; tab: SpectatorPanelTab; open: boolean; selectedPlayerId: string | null; eventFilter: string; onTab: (tab: SpectatorPanelTab) => void; onClose: () => void; onSelectPlayer: (id: string | null) => void; onFilter: (value: string) => void }) {
  const [chartKey, setChartKey] = useState<SpectatorChartKey>('assets'); const stats = createSpectatorPlayerStats(session); const selected = stats.find((entry) => entry.player.id === selectedPlayerId); const acted = new Set(session.turns.map((turn) => turn.playerId)).size
  const filteredEvents = session.spectatorEvents.filter((event) => eventFilter === 'all' || event.type === eventFilter || event.playerId === eventFilter).slice().reverse()
  return <aside className={cx('spectator-dashboard', open && 'is-open')} aria-label="观战数据面板"><header><div><small>拍卖大作战</small><strong>实时数据中心</strong></div><button aria-label="关闭数据面板" onClick={onClose}>×</button></header><nav>{([['battle', '战况'], ['data', '数据'], ['events', '事件']] as const).map(([id, label]) => <button key={id} className={cx(tab === id && 'is-active')} onClick={() => onTab(id)}>{label}</button>)}</nav><div className="spectator-dashboard__body">
    {tab === 'battle' && <><div className="spectator-progress"><span>本轮进度</span><strong>{acted} / {session.players.length}</strong><i><b style={{ width: `${acted / Math.max(1, session.players.length) * 100}%` }} /></i></div><div className="spectator-rank-list">{stats.map((entry) => <button key={entry.player.id} onClick={() => onSelectPlayer(entry.player.id)}><b>{entry.place}</b><span style={{ background: entry.player.color }}><PlayerAvatar player={entry.player} /></span><div><strong>{entry.player.name}</strong><small>{entry.player.identity ? getIdentityDefinition(entry.player.identity.id).name : '选择身份中'} · {entry.mainCategory}</small></div><em><CoinValue units={entry.totalAssetUnits} /><small>现金 {formatCoins(entry.cashUnits)} · 固资 {formatCoins(entry.fixedAssetUnits)}</small></em></button>)}</div></>}
    {tab === 'data' && <><div className="spectator-chart-tabs">{([['assets', '总资产'], ['cash', '现金'], ['bids', '累计下注'], ['net', '累计净收益']] as const).map(([id, label]) => <button key={id} className={cx(chartKey === id && 'is-active')} onClick={() => setChartKey(id)}>{label}</button>)}</div><SpectatorChart session={session} chartKey={chartKey} /><details className="spectator-detail-table"><summary>展开详细统计</summary>{stats.map((entry) => <button key={entry.player.id} onClick={() => onSelectPlayer(entry.player.id)}><strong>{entry.player.name}</strong><span>总下注 {formatCoins(entry.totalBidUnits)}</span><span>均注 {formatCoins(entry.averageBidUnits)}</span><span>获奖 {entry.awardCount} 次</span><span>预测 {entry.predictionHits}/{entry.predictionCount}</span><span>预测净值 {formatCoins(entry.predictionNetUnits)}</span><span>道具 {entry.cardUses} · 技能 {entry.skillUses}</span><span>道具市场 支 {formatCoins(entry.cardAuctionSpentUnits)} / 收 {formatCoins(entry.cardAuctionIncomeUnits)}</span><span>拍品市场 支 {formatCoins(entry.assetAuctionSpentUnits)} / 收 {formatCoins(entry.assetAuctionIncomeUnits)}</span></button>)}</details></>}
    {tab === 'events' && <><select className="spectator-event-filter" aria-label="筛选观战事件" value={eventFilter} onChange={(event) => onFilter(event.target.value)}><option value="all">全部事件</option><option value="turn">回合操作</option><option value="identityChoice">身份选择</option><option value="roundResult">回合结算</option>{session.players.map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select><div className="spectator-event-list">{filteredEvents.map((event) => <article key={event.id}><i>{event.sequence + 1}</i><div><strong>{event.summary}</strong><small>第 {event.roundIndex + 1} 轮 · {event.details.join(' · ')}</small></div></article>)}</div></>}
  </div>{selected && <div className="spectator-player-detail" role="dialog" aria-modal="true"><header><div><span style={{ background: selected.player.color }}><PlayerAvatar player={selected.player} /></span><div><small>玩家详情</small><h2>{selected.player.name}</h2></div></div><button onClick={() => onSelectPlayer(null)}>×</button></header><div className="spectator-player-detail__identity"><b className="game-art-slot">{selected.player.identity ? <IdentityIcon id={selected.player.identity.id} /> : '◇'}</b><div><strong>{selected.player.identity ? getIdentityDefinition(selected.player.identity.id).name : '未选身份'}</strong><small>库存 {selected.cardCount} 张 · 收藏 {selected.itemCount} 件</small></div></div><div className="spectator-player-detail__numbers"><span><b>{formatCoins(selected.cashUnits)}</b>现金</span><span><b>{formatCoins(selected.fixedAssetUnits)}</b>固定资产</span><span><b>{formatCoins(selected.totalAssetUnits)}</b>总资产</span></div><h3>收藏进度</h3>{selected.categories.map((entry) => <div className="spectator-category-row" key={entry.name}><strong>{entry.name}</strong><span>{entry.count} 件 · +{formatCoins(entry.units)}</span><small>{entry.nextAt ? `下一档 ${entry.nextAt} 件` : '最高档'}</small></div>)}<h3>逐轮操作</h3>{session.results.flatMap((result) => result.turns.map((turn) => ({ result, turn }))).filter(({ turn }) => turn.playerId === selected.player.id).map(({ result, turn }) => <div className="spectator-player-round" key={result.roundIndex}><b>第 {result.roundIndex + 1} 轮</b><span>下注 {formatCoins(turn.bidUnits)} · {turn.predictedPlayerId ? `预测 ${playerName(session.players, turn.predictedPlayerId)}` : '未预测'}</span></div>)}</div>}</aside>
}
...
function KidnapTargetPicker({
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="kidnap-target-title"><section className="target-picker-sheet kidnap-picker">
    ...
    <div className="target-picker-grid">{players.map((candidate) => {
      ...
      return <button key={candidate.id} disabled={locked} className={cx(selected && 'is-selected')} onClick={() => onToggle(candidate.id)}><span style={{ background: candidate.color }}><PlayerAvatar player={candidate} /></span><div><strong>{candidate.name}</strong><small>{selected ? '已加入名单' : locked ? `最多选择 ${maximum} 人` : '点击加入名单'}</small></div><i>{selected ? '✓' : '+'}</i></button>
    })}</div>
...
function LobbyistTaskPicker({ extraCost, onSelect, onClose }: { extraCost: number; onSelect: (taskType: LobbyistTaskType | null) => void; onClose: () => void }) {
  ...
  const taskSymbol = (task: LobbyistTaskType) => {
    const definition = LOBBYIST_TASKS.find((candidate) => candidate.type === task)
    return definition?.needsComparison ? '↔' : task === 'winFirst' ? 'Ⅰ' : task === 'winSecond' ? 'Ⅱ' : task === 'bidZero' ? '0' : '—'
  }
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="lobby-task-picker-title"><section className="target-picker-sheet lobby-task-picker">
    {step === 'mode' ? <div className="lobby-picker-step lobby-picker-step--mode">
      <p className="eyebrow">说客 · 发动技能</p><h2 id="lobby-task-picker-title">选择发布方式</h2><p>选完后会自动进入人物卡选择。基础发布费用照常计算，只有指定发布会额外收费。</p>
      <div className="lobby-mode-grid">
        <button className="lobby-mode-card lobby-mode-card--random" onClick={() => onSelect(null)}><span>?</span><div><strong>随机发布</strong><small>系统现在锁定一项任务，下一轮私密送达。</small></div><b className="status-token status-token--free">不额外支付</b><i>→</i></button>
        <button className="lobby-mode-card" onClick={() => setStep('specified')}><span>✦</span><div><strong>指定发布</strong><small>自己挑选想让对方完成的任务。</small></div><b className="status-token status-token--cost">额外支付 {extraCost} 金币</b><i>→</i></button>
      </div>
      <button className="button button--paper" onClick={onClose}>取消</button>
    </div> : <div className="lobby-picker-step lobby-picker-step--specified">
      <p className="eyebrow">说客 · 指定发布 · 2 / 2</p><h2 id="lobby-task-picker-title">选择指定任务</h2><p>选中后会自动进入人物卡选择。发布时会额外支付 <b className="status-token status-token--cost">{extraCost} 金币</b>。</p>
      <div className="lobby-task-grid">{LOBBYIST_TASKS.map((task) => <button key={task.type} className="lobby-task-card" onClick={() => onSelect(task.type)}><span>{taskSymbol(task.type)}</span><div><strong>{task.label}</strong><small>{task.detail}</small></div><i>→</i></button>)}</div>
      <div className="lobby-picker-actions"><button className="button button--paper" onClick={() => setStep('mode')}>返回发布方式</button><button className="button button--paper" onClick={onClose}>取消</button></div>
    </div>}
  </section></div>
}
...
function ProphetResult({ divination, session }: { divination: ProphetDivination; session: GameSession }) {
  ...
  const guesses = divination.identityGuesses ?? (divination.identityGuess ? [divination.identityGuess] : [])
    ...
    <div className="card-offer-grid" role="list" aria-label={title}>
      {cardIds.map((cardId) => {
        ...
        return <button key={cardId} type="button" role="listitem" className={cx('card-offer-choice', chosen && 'is-selected')} disabled={chosen} onClick={() => onChoose(cardId)}>
          <span className="game-art-slot card-offer-choice__symbol"><CardIcon id={card.id} /></span>
...
function PrivateTurn({ session, onInstantCard, onBuyLottery, onSubmit, onAcknowledgeGrant, onAcknowledgeNotice, onStartPrizeReroll, onChoosePrizeReroll, onConfirmPrizeReroll, onStartMerchantOffer, onChooseMerchantOffer, onOpenMerchantShop, onBuyMerchantShopCard, onChooseProphetOffer, onUseProphetDivination, onResolveFateCoin, onArmDeadline }: { session: GameSession; onInstantCard: (cardId: 'luckyTickets' | 'sleeveUpgrade', target?: CardId, planned?: CardUse[]) => boolean; onBuyLottery: (number: number | null, reservedUnits: number) => boolean; onSubmit: (turn: RoundTurn, timedOut?: boolean) => boolean; onAcknowledgeGrant: (playerId: string) => void; onAcknowledgeNotice: (noticeId: string) => void; onStartPrizeReroll: (playerId: string, cardId: 'prizeReroll' | 'prizeSwap') => void; onChoosePrizeReroll: (cardId: 'prizeReroll' | 'prizeSwap', itemId: string) => void; onConfirmPrizeReroll: (cardId: 'prizeReroll' | 'prizeSwap') => void; onStartMerchantOffer: (playerId: string) => void; onChooseMerchantOffer: (cardId: CardId) => void; onOpenMerchantShop: (playerId: string) => boolean; onBuyMerchantShopCard: (playerId: string, cardId: CardId) => boolean; onChooseProphetOffer: (playerId: string, cardId: CardId) => void; onUseProphetDivination: (playerId: string, mode: ProphetDivination['mode'], targetPlayerId?: string, identityId?: IdentityId) => boolean; onResolveFateCoin: (playerId: string, result: 'heads' | 'tails') => CardUse | null; onArmDeadline: () => void }) {
  ...
  const [nightwalkerDialog, setNightwalkerDialog] = useState(false)
  ...
  const [toolPanel, setToolPanel] = useState<PrivateToolPanel>(null)
  const [merchantShopPrompt, setMerchantShopPrompt] = useState(false)
  const [merchantShopPromptDismissed, setMerchantShopPromptDismissed] = useState(false)
  ...
  const lockedPrizeRerolls = session.pendingPrizeChanges.filter((change) => change.playerId === player.id && change.roundIndex === session.roundIndex)
  const lockedPrizeReroll = lockedPrizeRerolls.find((change) => !change.confirmedItemId) ?? null
  const lockedFateCoinUses = session.pendingFateCoinUse?.playerId === player.id && session.pendingFateCoinUse.roundIndex === session.roundIndex ? [...(session.pendingFateCoinUse.previousUses ?? []), session.pendingFateCoinUse.use] : []
  const prizeChangeUses: CardUse[] = lockedPrizeRerolls.filter((change) => Boolean(change.confirmedItemId)).map((change) => ({ cardId: change.cardId, prizeReroll: { originalItemId: change.originalItem.id, offeredItemIds: change.offeredItems.map((item) => item.id), chosenItemId: change.confirmedItemId as string, targetRoundIndex: change.targetRoundIndex } }))
  const [upgradeTarget, setUpgradeTarget] = useState<CardId | undefined>()
  const lockedInstantUses = instantUses(session, player.id)
  const [instantFeedback, setInstantFeedback] = useState<CardUse | null>(null)
  useEffect(() => { setInstantFeedback(lockedInstantUses.at(-1) ?? null) }, [lockedInstantUses.length])
  const allConfirmedCardUses = [...lockedInstantUses, ...confirmedCardUses.filter((use) => use.cardId !== 'fateCoin'), ...prizeChangeUses, ...lockedFateCoinUses]
  const canSubmitCards = !cardConfirming && targetPicker !== 'card' && !lockedPrizeRerolls.some((change) => !change.confirmedItemId)
  ...
  const confirmCardUse = () => {
    ...
    if (cardConfirming.cardId === 'luckyTickets' || cardConfirming.cardId === 'sleeveUpgrade') {
      if (onInstantCard(cardConfirming.cardId, upgradeTarget, confirmedCardUses)) { setCardConfirming(null); setToolPanel(null); setUpgradeTarget(undefined) }
      else setSubmitError('没有空闲号码或可升级卡，请检查选择。未消耗道具。')
      return
    }
    ...
    if (cardConfirming.cardId === 'prizeReroll' || cardConfirming.cardId === 'prizeSwap') {
      onStartPrizeReroll(player.id, cardConfirming.cardId)
      setCardConfirming(null)
      setToolPanel(null)
      return
    }
    if (cardConfirming.cardId === 'fateCoin') {
      setCardConfirming(null)
      setCoinFlipResult(null)
      setCoinFlipStarted(false)
      return
    }
    const use: CardUse = cardConfirming
    lockCardUse(use)
  }
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const lockedUse = onResolveFateCoin(player.id, result)
      if (!lockedUse) return
      setCoinFlipResult(result)
      setBidUnits((current) => Math.max(0, Math.min(current, player.balanceUnits + (lockedUse.fateDeltaUnits ?? 0))))
    }, 900)
    return () => window.clearTimeout(timer)
  }, [cardConfirming, coinFlipResult, coinFlipStarted, onResolveFateCoin, player.balanceUnits, player.id])
  useEffect(() => {
    if (identityAction?.type !== 'nightwalkerDoubleBid' || identityAction.shadowBidUnits <= player.balanceUnits) return
    if (player.balanceUnits < 1) {
      setIdentityAction(undefined)
      return
    }
    const nextBase = Math.min(bidUnits, player.balanceUnits - 1)
    setBidUnits(nextBase)
    setIdentityAction({ type: 'nightwalkerDoubleBid', shadowBidUnits: player.balanceUnits })
  }, [bidUnits, identityAction, player.balanceUnits])
  const identity = advancedToolsUnlocked ? player.identity : undefined
  const rankChangingCardIds: CardId[] = ['doubleBid', 'swap', 'bananaPeel', 'reverseRank']
  const nightwalkerActive = identityAction?.type === 'nightwalkerDoubleBid'
  const nightwalkerConflict = allConfirmedCardUses.some((use) => rankChangingCardIds.includes(use.cardId))
  const nightwalkerUses = identity?.nightwalkerUses ?? 0
  const thiefCardStealLimit = session.settings.identitySettings.thiefMaxSteals ?? 2
  ...
  const nightwalkerUnavailableReason = identity?.id !== 'nightwalker' ? null
    : nightwalkerUses >= session.settings.identitySettings.nightwalkerUseLimit ? `本局双影下注已用完（${nightwalkerUses}/${session.settings.identitySettings.nightwalkerUseLimit}）`
      : nightwalkerConflict ? '本轮已安排会改变排名下注的道具'
        : player.balanceUnits < 1 ? '至少要有 0.5 金币，才能设定两档不同下注' : null
  const visibleCardInventory = advancedToolsUnlocked ? player.cardInventory : []
  const currentProphetDivination = session.prophetDivinations.find((entry) => entry.playerId === player.id && entry.roundIndex === session.roundIndex && entry.mode !== 'identity')
  const currentProphetIdentityDivination = session.prophetDivinations.find((entry) => entry.playerId === player.id && entry.roundIndex === session.roundIndex && entry.mode === 'identity')
  const prophetIdentityGuesses = currentProphetIdentityDivination?.identityGuesses ?? (currentProphetIdentityDivination?.identityGuess ? [currentProphetIdentityDivination.identityGuess] : [])
  const prophetIdentityGuessesLeft = prophetIdentityGuessesRemaining(prophetIdentityGuesses)
  const prophetFutureItems = session.prophecyDeck.slice(session.roundIndex + 1, session.roundIndex + 3)
  const prophetProgressFor = (targetPlayerId: string) => getProphetIdentityProgress(session.prophetDivinations, player.id, targetPlayerId, session.prophetIdentityProgress[player.id]?.[targetPlayerId])
  const prophetTargetAlreadySolved = (targetPlayerId: string) => Boolean(prophetProgressFor(targetPlayerId).solvedIdentityId)
  const canUseIdentityDivination = prophetIdentityGuessesLeft > 0 && session.players.some((candidate) => candidate.id !== player.id && !prophetTargetAlreadySolved(candidate.id))
  ...
  const reverserFreeThisRound = identity?.reverserFreeRoundIndex === session.roundIndex
  const reverserCost = reverserFreeThisRound ? 0 : session.settings.identitySettings.reverserActivationCoins * (session.roundIndex >= session.settings.rounds - 2 ? 2 : 1)
  ...
  const thiefIsFree = thiefCostUnits === 0
  ...
  const pendingMerchantOffer = session.pendingMerchantOffers.find((offer) => offer.playerId === player.id && offer.roundIndex === session.roundIndex)
  const merchantShop = session.merchantShops.find((shop) => shop.playerId === player.id && shop.roundIndex === session.roundIndex)
  const merchantShopWindow = identity?.id === 'merchant'
  const merchantAuctionWindow = merchantShopWindow && session.roundIndex < session.settings.rounds - 2
  // 本轮商店与商人竞购是二选一：一旦已经锁定竞购候选，不能再开店；反之亦然。
  const merchantShopAvailable = Boolean(merchantShopWindow && !merchantShop && !pendingMerchantOffer && player.balanceUnits >= MERCHANT_SHOP_ENTRY_FEE_UNITS && Array.from(new Set(session.cardDeck)).length >= 3)
  const pendingProphetOffer = session.pendingProphetCardOffers.find((offer) => offer.playerId === player.id && offer.chosenCardIds.length < 2)
  ...
  const merchantActionChoiceAvailable = Boolean(merchantShopWindow && !merchantShop && !pendingMerchantOffer && (merchantShopAvailable || (merchantAuctionWindow && !merchantUnavailableReason)))
  const auctionLots = session.roundAuctions ?? []
  const assetAuctionLots = session.roundAssetAuctions ?? []
  const auctionBidTotal = Object.values(auctionBidByLotId).reduce((total, units) => total + units, 0)
  const reservedIdentityUnits = identityAction?.type === 'reverserInvert' ? reverserCostUnits : identityAction?.type === 'kidnap' ? kidnapCostUnits : identityAction?.type === 'thiefSteal' ? thiefCostUnits : identityAction?.type === 'invest' ? identityAction.investmentUnits : 0
  const merchantShopSpendableUnits = Math.max(0, player.balanceUnits - bidUnits - auctionBidTotal - reservedIdentityUnits)
  const merchantShopOpenAffordable = merchantShopSpendableUnits >= MERCHANT_SHOP_ENTRY_FEE_UNITS
  const auctionBudgetForLot = (lotId: string) => {
    const other = Object.entries(auctionBidByLotId).reduce((total, [id, units]) => total + (id === lotId ? 0 : units), 0)
    return Math.max(0, player.balanceUnits - bidUnits - reservedIdentityUnits - other)
  }
  const setAuctionBid = (lotId: string, value: number) => {
    const lot = auctionLots.find((entry) => entry.id === lotId)
    const assetLot = assetAuctionLots.find((entry) => entry.id === lotId)
    if ((!lot && !assetLot) || lot?.merchantId === player.id || assetLot?.sellerId === player.id) return
    const max = auctionBudgetForLot(lotId)
    const minimum = assetLot?.minimumBidUnits ?? 0
    if (assetLot && value > 0 && max < minimum) {
      setSubmitError(`可用余额不足起拍价 ${formatCoins(minimum)} 金币，不能参与「${assetLot.item.name}」竞购。`)
      return
    }
    setSubmitError(null)
    setAuctionBidByLotId((bids) => ({ ...bids, [lotId]: Math.max(0, Math.min(max, Math.max(value > 0 ? minimum : 0, Math.round(value)))) }))
  }
  ...
  const lobbyUnavailableReason = identity?.id !== 'lobbyist' ? null : activeSkillLimitReached ? `本局发布次数已用完（${activeSkillUses}/${activeSkillLimit}）` : session.roundIndex >= session.settings.rounds - 1 ? '最后一轮无法发布任务' : identity.lobbyistLastIssuedRound === session.roundIndex ? '本轮任务已发布' : null
  ...
  const submitBlockReason = !canSubmitCards
    ? lockedPrizeReroll ? '请先在改拍道具弹窗中选定拍品并确认使用。' : '请先完成或关闭正在进行的道具操作。'
    : !lobbyActionValid ? '说客任务尚未选择完整目标。'
      : identityAction?.type === 'lobbyistContract' && lobbyShortfallUnits > 0 ? '余额不足以支付本次说客任务费用。'
        : null
  const assetBreakdowns = calculateFixedAssets(player.items, identity?.id === 'collector' ? identity.collectorCategory : undefined)
  const fixedAssets = assetBreakdowns.filter((asset) => asset.itemCount > 0)
  ...
  useEffect(() => {
    if (operationReady && merchantActionChoiceAvailable && !merchantShopPrompt && !merchantShopPromptDismissed) setMerchantShopPrompt(true)
  }, [merchantActionChoiceAvailable, merchantShopPrompt, merchantShopPromptDismissed, operationReady])
  const focusFlowActive = Boolean(cardConfirming || targetPicker || identityConfirming || nightwalkerDialog || prophetDialog || confirming || peekResult || merchantShopPrompt || (pendingMerchantOffer && !pendingMerchantOffer.chosenCardId) || pendingProphetOffer || visibleGrant || activeNotice)
  const identityDockStatus = !identity ? '身份关闭'
    : identitySkillMode(identity.id) === 'passive' ? '被动生效'
      : identityAction ? '本轮已安排'
        : identity.id === 'nightwalker' && nightwalkerUnavailableReason ? nightwalkerUnavailableReason
          : identity.id === 'merchant' && merchantUnavailableReason ? merchantUnavailableReason
            : identity.id === 'lobbyist' && lobbyUnavailableReason ? lobbyUnavailableReason
              : activeSkillLimitReached ? '次数已用完'
                : '可发动技能'
  const assetTierWindow = (category: AssetCategory, count: number) => {
    const start = count <= 2 ? 1 : count - 1
    return [start, start + 1, start + 2].map((tierCount) => ({
      count: tierCount,
      label: tierCount === 1 ? '单件计价' : `+${fixedAssetCoins(category, tierCount)}`,
    }))
  }
  useEffect(() => {
  ...
  useEffect(() => {
    setToolPanel(null)
  }, [player.id, session.roundIndex])
  useEffect(() => {
    if (!toolPanel || focusFlowActive) return
    const previousFocus = document.activeElement as HTMLElement | null
    const sheet = document.querySelector<HTMLElement>('.focus-backdrop .focus-sheet')
    const focusable = () => Array.from(sheet?.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), [tabindex]:not([tabindex="-1"])') ?? [])
    focusable()[0]?.focus()
  ...
  const submitCurrentChoice = (timedOut = false) => {
    ...
    const invalidAssetBid = auctionBids.find((bid) => {
      const lot = assetAuctionLots.find((entry) => entry.id === bid.lotId)
      return lot && bid.bidUnits > 0 && bid.bidUnits < lot.minimumBidUnits
    })
    if (invalidAssetBid) {
      const lot = assetAuctionLots.find((entry) => entry.id === invalidAssetBid.lotId)!
      setConfirming(false)
  ...
  return (
    <section className="private-turn">
      <div className="private-heading"><div><p className="eyebrow">第 {session.roundIndex + 1} 轮 · {player.name}</p><h1>你的回合</h1></div><div className="private-overview"><BalanceReveal units={player.balanceUnits} /></div></div>
      ...
      {submitError && <div className="private-submit-error" role="alert"><strong>暂时不能提交</strong><span>{submitError}</span><button aria-label="关闭提示" onClick={() => setSubmitError(null)}>×</button></div>}
      ...
      <div className="turn-grid">
        <div className="bid-panel panel">
          ...
          <div className="bid-control">
            ...
            <input className="range range--bid" aria-label="秘密下注" type="range" min="0" max={Math.max(0, player.balanceUnits - (identityAction?.type === 'reverserInvert' ? reverserCostUnits : identityAction?.type === 'kidnap' ? kidnapCostUnits : identityAction?.type === 'thiefSteal' ? thiefCostUnits : identityAction?.type === 'invest' ? identityAction.investmentUnits : 0))} step="1" value={bidUnits} disabled={nightwalkerActive} onChange={(event) => setBid(Number(event.target.value))} />
            <div className="bid-shortcuts"><button disabled={nightwalkerActive} onClick={() => setBid(bidUnits - 1)}>−0.5</button><button disabled={nightwalkerActive} onClick={() => setBid(bidUnits + 1)}>+0.5</button><button disabled={nightwalkerActive} onClick={() => setBid(bidUnits + 2)}>+1</button><button disabled={nightwalkerActive} onClick={() => setBid(bidUnits + 10)}>+5</button><button disabled={nightwalkerActive} onClick={() => setBid(player.balanceUnits)}>全部</button></div>
          </div>
        </div>
        <div className={cx('prediction-panel prediction-panel--compact panel', !predictionUnlocked && 'is-locked')}>
          <div className="panel-title"><div><p className="eyebrow">可选</p><h2>预测第一</h2></div><span>猜中 +{item.value * session.settings.correctPredictionMultiplier}{identity?.id === 'gambler' ? ` + ${item.value * session.settings.identitySettings.gamblerCorrectBonusMultiplier}` : ''}<br />{identity?.id === 'gambler' ? `猜错 −${item.value * session.settings.identitySettings.gamblerWrongPenaltyMultiplier} · 跳过 −${item.value * session.settings.identitySettings.gamblerSkipPenaltyMultiplier}` : `猜错 −${item.value * session.settings.wrongPredictionMultiplier}`}</span></div>
          {!predictionUnlocked ? <p className="tutorial-locked-copy">下一轮解锁预测。</p> : <div className="prediction-actions" role="group" aria-label="预测第一名">
            <button className={cx(prediction === null && 'is-selected')} onClick={() => setPrediction(null)}><span>—</span><div><strong>不预测</strong><small>保持当前选择</small></div><i>{prediction === null ? '✓' : ''}</i></button>
            <button data-testid="prediction-picker" className={cx(prediction !== null && 'is-selected')} onClick={() => setToolPanel('prediction')}><span style={{ background: predicted?.color ?? '#687c9b' }}>{predicted ? <PlayerAvatar player={predicted} /> : '?'}</span><div><strong>{predicted?.name ?? '选择玩家'}</strong><small>{predicted ? '点击更换预测对象' : '打开人物卡'}</small></div><i>→</i></button>
          </div>}
        </div>
      </div>
      ...
      {(auctionLots.length > 0 || assetAuctionLots.length > 0) && <section className="round-auctions turn-market panel" aria-label="本轮市场">
        ...
        <div className="round-auctions__rail">{auctionLots.map((lot) => {
          const card = getCardDefinition(lot.cardId)
          const locked = lot.merchantId === player.id
          const bid = locked ? 0 : auctionBidByLotId[lot.id] ?? 0
          return <article className={cx('round-auction-card', locked && 'is-own')} key={lot.id}><span className="game-art-slot"><CardIcon id={card.id} /></span><strong>{card.name} <CardRarityTag cardId={card.id} /></strong><small>{locked ? '这是你安排的竞购，本人不可报价。' : card.description}</small><div><button disabled={locked || bid <= 0} onClick={() => setAuctionBid(lot.id, bid - 1)}>−</button><b><CoinValue units={bid} /></b><button disabled={locked} onClick={() => setAuctionBid(lot.id, bid + 1)}>+</button></div></article>
        })}</div></div>}
        {assetAuctionLots.length > 0 && <div className="turn-market__group"><div className="turn-market__label"><strong>藏品竞购</strong><small>达到起拍价后参与，最高唯一报价成交</small></div>
        <div className="round-auctions__rail">{assetAuctionLots.map((lot) => {
          const locked = lot.sellerId === player.id
          const bid = locked ? 0 : auctionBidByLotId[lot.id] ?? 0
          const category = categoryConfig(lot.item.category)
          const available = auctionBudgetForLot(lot.id)
          const cannotMeetReserve = !locked && bid === 0 && available < lot.minimumBidUnits
          return <article className={cx('round-auction-card', 'round-auction-card--asset', locked && 'is-own', cannotMeetReserve && 'is-unaffordable')} key={lot.id}><span>{lot.item.emoji}</span><strong>{lot.item.name}</strong><small>{locked ? '这是你发起的竞购，自己不能报价。' : `${category.name} · 藏品竞购`}</small>{!locked && <div className="auction-starting-price"><span>起拍价</span><b>{formatCoins(lot.minimumBidUnits)} 金币</b></div>}{cannotMeetReserve && <small className="auction-budget-warning">可用余额不足起拍价，无法报价。</small>}<div><button disabled={locked || bid <= 0} onClick={() => setAuctionBid(lot.id, bid <= lot.minimumBidUnits ? 0 : bid - 1)}>−</button><b><CoinValue units={bid} /></b><button disabled={locked || (bid === 0 && available < lot.minimumBidUnits) || (bid > 0 && available <= bid)} onClick={() => setAuctionBid(lot.id, bid === 0 ? lot.minimumBidUnits : bid + 1)}>+</button></div></article>
        })}</div></div>}
      </section>}
      <button type="button" className="asset-summary-strip" onClick={() => setToolPanel('assets')} aria-label="查看我的资产详情">
        <span className="asset-summary-strip__lead"><i>◆</i><span><strong>我的资产</strong><small>{player.items.length} 件藏品 · 加成 {formatCoins(assetBreakdowns.reduce((total, asset) => total + asset.units, 0))}</small></span></span>
        <span className="asset-summary-strip__categories">{ASSET_CATEGORY_CONFIGS.map((config) => { const asset = assetBreakdowns.find((entry) => entry.category === config.category)!; const tiers = assetTierWindow(config.category, asset.itemCount); return <span className="asset-category-summary" key={config.category}><strong>{config.name}<b>{asset.itemCount} 件</b></strong><small>当前 +{formatCoins(asset.units)}</small><em>{tiers.map((tier) => <i key={tier.count} className={tier.count === asset.itemCount ? 'is-current' : undefined}>{tier.count}件 {tier.label}</i>)}</em></span> })}</span>
        <em>查看 →</em>
      </button>
      {toolPanel === 'assets' && !focusFlowActive && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="assets-tool-title"><section className="focus-sheet focus-sheet--assets private-assets">
        <header className="focus-sheet__head"><div><p className="eyebrow">资产</p><h2 id="assets-tool-title">我的收藏</h2><small>同类越多，终局加成越高</small></div><button className="icon-button" aria-label="关闭资产" onClick={() => setToolPanel(null)}>×</button></header>
      ...
      {toolPanel === 'identity' && !focusFlowActive && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="identity-tool-title"><section className="focus-sheet focus-sheet--identity identity-skills">
        <header className="focus-sheet__head"><div><p className="eyebrow">身份与技能</p><h2 id="identity-tool-title">{identity ? getIdentityDefinition(identity.id).name : '身份未启用'}</h2><small>{identity ? identitySkillMode(identity.id) === 'active' ? '查看状态并发动本轮技能' : '被动能力会自动生效' : '本局没有身份能力'}</small></div><button className="icon-button" aria-label="关闭身份技能" onClick={() => setToolPanel(null)}>×</button></header>
        {!identity ? <div className="identity-skill-placeholder"><span>◎</span><div><strong>本局未启用身份系统</strong><small>下局可在高级设置中开启。</small></div></div> : <div className="identity-live">
          <div className="identity-live-head"><span className="game-art-slot"><IdentityIcon id={identity.id} /></span><div><strong>{getIdentityDefinition(identity.id).name}</strong><small>{getIdentityDefinition(identity.id).summary}</small></div></div>
          ...
          {identity.id === 'nightwalker' && <div className="nightwalker-skill"><p><strong>双影下注</strong>是主动技能：本局还可使用 <b className="status-token status-token--count">{Math.max(0, session.settings.identitySettings.nightwalkerUseLimit - nightwalkerUses)} 次</b>。设定明面 A 与更高的夜行影价 B；所有人提交后，系统会比较“排名奖励 − 实际下注”。可选 <b>优先拿藏品</b>：仅一档能拿到拍品时优先选它；绑匪抢夺仍无法预知。</p><button className={cx('button', nightwalkerActive && 'button--primary')} disabled={Boolean(nightwalkerUnavailableReason)} onClick={() => { if (nightwalkerActive) { setIdentityAction(undefined); return } setNightwalkerBaseBidUnits(Math.min(bidUnits, Math.max(0, player.balanceUnits - 1))); setNightwalkerShadowBidUnits(Math.max(1, Math.min(player.balanceUnits, bidUnits + 1))); setNightwalkerPrioritizeItem(true); setNightwalkerDialog(true) }}>{nightwalkerActive ? `已设双影：A ${formatCoins(bidUnits)} / B ${formatCoins(identityAction.shadowBidUnits)} · 点击撤销` : nightwalkerUnavailableReason ?? '设定双影下注（免费）'}</button></div>}
          {identity.id === 'merchant' && <><p>{merchantShop ? '本轮已选择道具商店；未购买的道具会在提交后回到卡池。' : pendingMerchantOffer?.chosenCardId ? `本轮已安排「${getCardDefinition(pendingMerchantOffer.chosenCardId).name}」进入下轮竞购。` : merchantAuctionWindow ? <>本轮可二选一：发起下轮道具竞购，或支付 <b className="status-token status-token--count">2 金币</b> 进入道具商店。</> : <>最后两轮仅开放道具商店；入场费 <b className="status-token status-token--count">2 金币</b>。</>}</p>{merchantShop ? <button className="button button--primary" onClick={() => setToolPanel('merchantShop')}>{merchantShop.cards.length > 0 ? `道具商店 · 剩余 ${merchantShop.cards.length} 张` : '道具商店已售罄'}</button> : pendingMerchantOffer ? <button className="button button--primary" disabled>{pendingMerchantOffer.chosenCardId ? `已安排 ${getCardDefinition(pendingMerchantOffer.chosenCardId).name}` : '正在选择下轮竞购卡'}</button> : <div className="merchant-action-buttons">{merchantAuctionWindow && <button className="button" disabled={Boolean(merchantUnavailableReason)} onClick={() => onStartMerchantOffer(player.id)}>{merchantUnavailableReason ?? '发起道具竞购'}</button>}<button className="button button--primary" disabled={!merchantShopAvailable || !merchantShopOpenAffordable} onClick={() => { if (onOpenMerchantShop(player.id)) { setMerchantShopPrompt(false); setToolPanel('merchantShop') } }}>{merchantShopAvailable && merchantShopOpenAffordable ? '进入道具商店 · 2 金币' : '需先为本轮下注保留金币'}</button></div>}</>}
      ...
      {merchantShopPrompt && !merchantShop && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="merchant-shop-open-title"><section className="focus-sheet focus-sheet--backpack merchant-shop-prompt">
        <header className="focus-sheet__head"><div><p className="eyebrow">道具商人 · 本轮选择</p><h2 id="merchant-shop-open-title">{merchantAuctionWindow ? '竞拍支持与道具商店已开放' : '道具商店已开放'}</h2><small>{merchantAuctionWindow ? '本轮只能选择其中一项；安排竞购后不能再进商店。' : '最后两轮不能再安排下轮竞购。'}</small></div></header>
        <div className="merchant-shop-prompt__coin">◆ <strong>2 金币</strong><small>一次性入场费</small></div>
        <p>{merchantAuctionWindow ? '发起竞购会锁定三张候选，从中挑一张安排到下一轮；进入商店后可买任意多张，未购买的卡会在提交时回池。' : '商店会锁定三张带价道具；可买任意多张，未购买的卡会在提交时回池。'}</p>
        <footer><button className="button button--paper" onClick={() => { setMerchantShopPrompt(false); setMerchantShopPromptDismissed(true) }}>稍后决定</button>{merchantAuctionWindow && <button className="button" disabled={Boolean(merchantUnavailableReason)} onClick={() => { onStartMerchantOffer(player.id); setMerchantShopPrompt(false); setMerchantShopPromptDismissed(true) }}>{merchantUnavailableReason ?? '发起道具竞购'}</button>}<button className="button button--primary" disabled={!merchantShopOpenAffordable} onClick={() => { if (onOpenMerchantShop(player.id)) { setMerchantShopPrompt(false); setToolPanel('merchantShop') } }}>{merchantShopOpenAffordable ? '进入商店' : '本轮已预留的金币不足'}</button></footer>
      </section></div>}
      {toolPanel === 'merchantShop' && merchantShop && !focusFlowActive && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="merchant-shop-title"><section className="focus-sheet focus-sheet--backpack merchant-shop">
        <header className="focus-sheet__head"><div><p className="eyebrow">道具商人 · 私密商店</p><h2 id="merchant-shop-title">挑选你的道具</h2><small>剩余金币 {formatCoins(player.balanceUnits)}；每张价格已锁定。</small></div><button className="icon-button" aria-label="关闭道具商店" onClick={() => setToolPanel(null)}>×</button></header>
        {merchantShop.cards.length === 0 ? <p className="empty-cards">商店已售罄。购买的道具已放入背包。</p> : <div className="merchant-shop__cards">{merchantShop.cards.map((entry) => { const card = getCardDefinition(entry.cardId); const affordable = merchantShopSpendableUnits >= entry.priceUnits; return <article key={entry.cardId} className={`merchant-shop__card merchant-shop__card--${card.rarity}`}><span className="game-art-slot"><CardIcon id={card.id} /></span><div><strong>{card.name} <CardRarityTag cardId={card.id} /></strong><small>{card.description}</small></div><button className="button" disabled={!affordable} onClick={() => onBuyMerchantShopCard(player.id, entry.cardId)}>{affordable ? `购买 ${formatCoins(entry.priceUnits)} 金币` : `还差 ${formatCoins(entry.priceUnits - merchantShopSpendableUnits)} 金币`}</button></article> })}</div>}
        <footer><small>可直接离开；未购买卡会在提交本回合时回到卡池。</small><button className="button button--primary" onClick={() => setToolPanel(null)}>继续操作</button></footer>
      </section></div>}
      {toolPanel === 'backpack' && !focusFlowActive && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="backpack-tool-title"><section className="focus-sheet focus-sheet--backpack card-inventory">
        <header className="focus-sheet__head"><div className="backpack-title"><span className="backpack-title__icon" aria-hidden="true"><i /><b>▥</b></span><div><p className="eyebrow">道具背包</p><h2 id="backpack-tool-title">选择一张卡</h2><small>{visibleCardInventory.length} 张库存 · 本轮已安排 {allConfirmedCardUses.length} 张</small></div></div><button className="icon-button" aria-label="关闭背包" onClick={() => setToolPanel(null)}>×</button></header>
        <div className="backpack-queued">{cardInventoryCounts(confirmedCardUses.filter(use => canStackCard(use.cardId) && !['fateCoin','peek'].includes(use.cardId)).map(use => use.cardId)).map(({cardId,count}) => <button type="button" key={cardId} className="button button--paper" aria-label={`撤销一张${getCardDefinition(cardId).name}`} onClick={() => setConfirmedCardUses(uses => { const index = uses.findIndex(use => use.cardId === cardId); return uses.filter((_, i) => i !== index) })}>{getCardDefinition(cardId).name} ×{count} · 撤销一张</button>)}</div>
        {visibleCardInventory.length === 0 ? <p className="empty-cards">{tutorial && !advancedToolsUnlocked ? '第 3 轮会解锁一张简单道具卡；先专注这一轮的新选择。' : '暂时没有道具卡。落后时，下一轮可能得到秘密支援。'}</p> : <div className="card-list">{cardInventoryCounts(visibleCardInventory).map(({ cardId, count }) => {
          ...
          const prizeChangeLocked = (cardId === 'prizeReroll' || cardId === 'prizeSwap') && lockedPrizeRerolls.some((change) => change.cardId === cardId)
          const prizeRerollBusy = cardId === lockedPrizeReroll?.cardId
          const passiveShield = cardId === 'reflectShield' || cardId === 'tieCharm'
          const nonCancelable = ['fateCoin', 'peek', 'luckyTickets', 'sleeveUpgrade'].includes(cardId)
          const stackable = canStackCard(cardId)
          const usedCount = allConfirmedCardUses.filter((use) => use.cardId === cardId).length
          const remaining = cardId === 'fateCoin' || cardId === 'sleeveUpgrade' ? count : count - usedCount
          return <button key={cardId} className={cx('card-choice', `card-choice--${card.rarity}`, (confirmed || prizeRerollBusy) && 'is-selected')} disabled={passiveShield || (nonCancelable && !stackable && Boolean(confirmed)) || (stackable && remaining <= 0) || (!confirmed && (unavailable || prizeRerollUnavailable || prizeChangeLocked || (nightwalkerActive && rankChangingCardIds.includes(cardId))))} onClick={() => { if (confirmed && !stackable) { setConfirmedCardUses((uses) => uses.filter((use) => use.cardId !== cardId)); return } openCardConfirmation({ cardId }) }}><span className="game-art-slot"><CardIcon id={card.id} /></span><div><strong>{card.name} <CardRarityTag cardId={cardId} />{count > 1 && <b className="card-choice__count">×{count}</b>}</strong><small>{cardId === 'tieCharm' ? card.description : passiveShield ? '自动防御：受到香蕉皮或偷天换日影响时自动反弹并消耗。' : stackable && usedCount > 0 ? `本轮已安排 ${usedCount} 张 · 还可使用 ${Math.max(0, remaining)} 张` : confirmed ? cardId === 'fateCoin' ? '硬币结果已锁定，本轮不能重掷。' : cardId === 'peek' ? '已查看投资额，本轮使用已锁定。' : '本轮已安排，点击取消。' : nightwalkerActive && rankChangingCardIds.includes(cardId) ? '已发动双影下注，本轮不能搭配改变排名下注的效果。' : prizeRerollUnavailable ? '最后一轮没有下一轮拍品，无法使用。' : prizeChangeLocked ? '已锁定一张改拍道具，选择后才能继续。' : unavailable ? '本轮尚无可选目标，可留到后续回合使用。' : card.description}</small></div><i>{(confirmed || prizeRerollBusy) ? '✓' : ''}</i></button>
        })}</div>}
      </section></div>}
      {lockedPrizeReroll && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-label={`${lockedPrizeReroll.cardId === 'prizeSwap' ? '调包令' : '改拍令'}选择`}><section className="focus-sheet focus-sheet--backpack prize-reroll-picker">
        <header className="focus-sheet__head"><div><p className="eyebrow">候选已锁定</p><h2>{lockedPrizeReroll.cardId === 'prizeSwap' ? '调包令：选择本轮拍品' : '改拍令：选择下一轮拍品'}</h2><small>{lockedPrizeReroll.chosenItemId ? '已选中一件；确认前仍可更换。' : `${lockedPrizeReroll.offeredItems.length} 张候选已经锁定，请选择一件。`}</small></div></header>
        <p>{lockedPrizeReroll.cardId === 'prizeSwap' ? '选定后，本轮实际拍品会被替换；其他玩家仍会看到原拍品，直到回合结算。' : '选定后会替换下一轮拍品；预言家仍看到原预设拍品。'} 抽取和选择都不能取消或重抽。</p>
        <div className="prize-reroll-options">{lockedPrizeReroll.offeredItems.map((candidate) => <button key={candidate.id} className={cx('prize-reroll-option', lockedPrizeReroll.chosenItemId === candidate.id && 'is-selected')} onClick={() => onChoosePrizeReroll(lockedPrizeReroll.cardId, candidate.id)}><span>{candidate.emoji}</span><strong>{candidate.name}</strong><small>{categoryConfig(candidate.category).name} · 价值 {candidate.value}</small></button>)}</div>
        <footer className="prize-reroll-picker__actions"><small>{lockedPrizeReroll.chosenItemId ? '确认后不可更换，且会记入本轮道具。' : '请先选中一件拍品。'}</small><button className="button button--primary" disabled={!lockedPrizeReroll.chosenItemId} onClick={() => onConfirmPrizeReroll(lockedPrizeReroll.cardId)}>确认使用</button></footer>
      </section></div>}
      {toolPanel === 'prediction' && !focusFlowActive && <div className="modal-backdrop focus-backdrop" role="dialog" aria-modal="true" aria-labelledby="prediction-tool-title"><section className="focus-sheet focus-sheet--prediction">
        <header className="focus-sheet__head"><div><p className="eyebrow">预测第一</p><h2 id="prediction-tool-title">你看好谁？</h2><small>选中人物卡后立即返回操作页</small></div><button className="icon-button" aria-label="关闭预测选择" onClick={() => setToolPanel(null)}>×</button></header>
        <div className="prediction-picker-grid">{session.players.filter((candidate) => candidate.id !== player.id).map((candidate, index) => { const investedTarget = identityAction?.type === 'invest' && identityAction.targetPlayerId === candidate.id; return <button key={candidate.id} disabled={investedTarget} className={cx(prediction === candidate.id && 'is-selected')} onClick={() => { setPrediction(candidate.id); setToolPanel(null) }} style={{ '--player-color': candidate.color, '--card-index': index } as React.CSSProperties}><span><PlayerAvatar player={candidate} /></span><div><strong>{candidate.name}</strong><small>{investedTarget ? '已投资该玩家，不能预测' : prediction === candidate.id ? '当前预测' : '选择为第一名'}</small></div><i>{prediction === candidate.id ? '✓' : '→'}</i></button> })}</div>
        <button className="button button--paper" onClick={() => { setPrediction(null); setToolPanel(null) }}>本轮不预测</button>
      </section></div>}
      ...
      <div className="private-command-bar">
        <nav className="turn-tool-dock" aria-label="回合工具">
          <button type="button" className={cx(identityAction && 'has-action')} onClick={() => setToolPanel('identity')} data-testid="identity-tool"><span className="game-art-slot">{identity ? <IdentityIcon id={identity.id} /> : '◎'}</span><div><strong>身份</strong><small>{identityDockStatus}</small></div>{identityAction && <i>1</i>}</button>
      ...
      {confirming && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="confirm-title">
          <div className="confirm-sheet">
            ...
            <div className="confirm-summary"><span>秘密下注 <strong><CoinValue units={bidUnits} /></strong></span><span>预测第一 <strong>{predicted?.name ?? '不预测'}</strong></span><span>使用道具 <strong>{[...allConfirmedCardUses.map((use) => `${getCardDefinition(use.cardId).name}${use.targetPlayerId ? ` · ${playerName(session.players, use.targetPlayerId)}` : ''}`), ...(lockedPrizeReroll ? ['改拍令'] : [])].join('、') || '不使用'}</strong></span></div>
      ...
      {nightwalkerDialog && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="nightwalker-title"><section className="confirm-sheet nightwalker-sheet"><p className="eyebrow">夜行者 · 双影下注</p><h2 id="nightwalker-title">秘密设定两档下注</h2><p>系统会在所有人提交后，分别模拟 A 和 B 的获奖名次。默认先判断是否能获得拍品；若两档都能或都不能获得拍品，再按“排名奖励 − 实际下注”选择更高者，同分保留 A。此技能 <b className="status-token status-token--free">免费</b>，本局还能使用 <b className="status-token status-token--count">{Math.max(0, session.settings.identitySettings.nightwalkerUseLimit - nightwalkerUses)} 次</b>。</p><label className="field-label">明面投注 A <strong><CoinValue units={nightwalkerBaseBidUnits} /></strong></label><input className="range range--bid" type="range" min="0" max={Math.max(0, player.balanceUnits - 1)} step="1" value={nightwalkerBaseBidUnits} onChange={(event) => { const next = Number(event.target.value); setNightwalkerBaseBidUnits(next); setNightwalkerShadowBidUnits((shadow) => Math.max(next + 1, Math.min(player.balanceUnits, shadow))) }} /><label className="field-label">夜行影价 B <strong><CoinValue units={nightwalkerShadowBidUnits} /></strong></label><input className="range range--bid" type="range" min={Math.min(player.balanceUnits, nightwalkerBaseBidUnits + 1)} max={player.balanceUnits} step="1" value={Math.max(nightwalkerBaseBidUnits + 1, nightwalkerShadowBidUnits)} onChange={(event) => setNightwalkerShadowBidUnits(Number(event.target.value))} /><label className="nightwalker-priority"><input type="checkbox" checked={nightwalkerPrioritizeItem} onChange={(event) => setNightwalkerPrioritizeItem(event.target.checked)} /><span><strong>优先拿藏品</strong><small>仅一档能获得本轮拍品时，优先采用那一档，即使金币净收益更低。绑匪是否抢走拍品不会参与模拟。</small></span></label><small className="nightwalker-note">B 必须高于 A，最多可设为当前全部可用余额。确认后可在本轮提交前撤销并重新设定。</small><div><button className="button button--paper" onClick={() => setNightwalkerDialog(false)}>取消</button><button className="button button--primary" onClick={() => { if (nightwalkerShadowBidUnits <= nightwalkerBaseBidUnits || nightwalkerShadowBidUnits > player.balanceUnits) return; setBidUnits(nightwalkerBaseBidUnits); setIdentityAction({ type: 'nightwalkerDoubleBid', shadowBidUnits: nightwalkerShadowBidUnits, prioritizeItem: nightwalkerPrioritizeItem }); setNightwalkerDialog(false) }}>确认双影下注</button></div></section></div>}
      {prophetDialog === 'menu' && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="target-picker-sheet prophet-picker"><p className="eyebrow">预言家 · 天机推演</p><h2>选择一种推演</h2><p>本轮在观财和观星中选择一项，均免费；观身份在技能区单独发动。</p><div className="prophet-option-grid"><button onClick={() => { if (onUseProphetDivination(player.id, 'wealth')) setProphetDialog(null) }}><span>◒</span><strong>观财</strong><small>免费：读取本轮开始时的全场最高、最低余额区间。</small></button><button disabled={prophetFutureItems.length === 0} onClick={() => { if (onUseProphetDivination(player.id, 'stars')) setProphetDialog(null) }}><span>✦</span><strong>观星</strong><small>{prophetFutureItems.length > 0 ? '免费：查看未来至多两轮的预设拍品。' : '最后一轮没有未来拍品。'}</small></button></div><button className="button button--paper" onClick={() => setProphetDialog(null)}>取消使用</button></section></div>}
      {prophetDialog === 'target' && <PlayerTargetPicker title="选择要观测身份的玩家" detail={`本轮还可猜 ${prophetIdentityGuessesLeft} 人；猜对会把机会重置为 2 次。每位玩家只提供 6 个固定候选，猜错会排除该身份。`} players={session.players.filter((candidate) => candidate.id !== player.id && !prophetTargetAlreadySolved(candidate.id))} selectedPlayerId={prophetTargetId} onSelect={(id) => { setProphetTargetId(id); setProphetDialog('identity') }} onClose={() => setProphetDialog(null)} />}
      {prophetDialog === 'identity' && <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="target-picker-sheet prophet-picker"><p className="eyebrow">预言家 · 观身份</p><h2>免费猜测 {playerName(session.players, prophetTargetId)} 的身份</h2><p>以下候选已在开局时锁定，其中包含真实身份。猜对会立即获得一张道具，且不能再猜这位玩家。</p><div className="identity-choice-grid prophet-identity-grid">{(session.prophetIdentityCandidates[player.id]?.[prophetTargetId] ?? []).filter((candidateId) => !prophetProgressFor(prophetTargetId).excludedIdentityIds.includes(candidateId)).map((candidateId) => { const definition = getIdentityDefinition(candidateId); const allowed = canMakeIdentityGuess(session.prophetDivinations, player.id, prophetTargetId, candidateId) && !prophetProgressFor(prophetTargetId).excludedIdentityIds.includes(candidateId); return <button key={candidateId} disabled={!allowed} className="identity-choice-card" onClick={() => { if (onUseProphetDivination(player.id, 'identity', prophetTargetId, candidateId)) setProphetDialog(null) }}><span className="game-art-slot"><IdentityIcon id={definition.id} /></span><h2>{definition.name}</h2><p>确认猜测此身份</p></button> })}</div><button className="button button--paper" onClick={() => setProphetDialog('target')}>返回选人</button></section></div>}
      {instantFeedback && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="道具使用结果"><section className="confirm-sheet"><h2>{instantFeedback.upgradedTo ? `获得${getCardDefinition(instantFeedback.upgradedTo).name}` : '获得彩票号码'}</h2>{instantFeedback.upgradedTo ? <span className="game-art-slot"><CardIcon id={instantFeedback.upgradedTo} /></span> : <p>{instantFeedback.lotteryNumbers?.join(' / ')} 号 · 奖池增加 {(instantFeedback.lotteryNumbers?.length ?? 0) * 2} 金币</p>}<button className="button button--primary" onClick={() => setInstantFeedback(null)}>收下</button></section></div>}
      {cardConfirming && targetPicker !== 'card' && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="card-confirm-title"><div className="confirm-sheet card-use-confirm"><p className="eyebrow">{cardConfirming.cardId === 'fateCoin' ? coinFlipStarted ? '命运硬币' : '准备使用道具' : '准备使用道具'}</p><h2 id="card-confirm-title">{getCardDefinition(cardConfirming.cardId).name} <CardRarityTag cardId={cardConfirming.cardId} /></h2>{cardConfirming.cardId === 'sleeveUpgrade' ? <div><p>选择一张卡，确认后随机升级，无法撤销。</p><div className="card-list upgrade-card-list">{upgradeChoices(session, player.id, confirmedCardUses).map(id => <button type="button" key={id} className={cx('card-choice', upgradeTarget === id && 'is-selected')} onClick={() => setUpgradeTarget(id)}><span className="game-art-slot"><CardIcon id={id} /></span><div><strong>{getCardDefinition(id).name}</strong><CardRarityTag cardId={id} /></div><i>{upgradeTarget === id ? '✓' : ''}</i></button>)}</div>{!upgradeChoices(session, player.id, confirmedCardUses).length && <p>该档卡池暂时为空，或没有可升级的卡。未消耗道具。</p>}</div> : cardConfirming.cardId === 'luckyTickets' ? <p>随机获得至多 {session.roundIndex >= session.settings.rounds - 1 ? 3 : 5} 个空闲号码，每号向奖池注入 2 金币。不扣余额，不占购票机会。</p> : cardConfirming.cardId === 'fateCoin' ? <div className="fate-coin-wrap"><div className={cx('fate-coin', coinFlipResult && 'is-settled')}><span>{coinFlipResult === 'heads' ? '正' : coinFlipResult === 'tails' ? '反' : '?'}</span></div><p>{!coinFlipStarted ? '确认后才会掷出硬币；结果出现后不能取消或重掷。' : coinFlipResult === null ? '硬币正在翻转…' : coinFlipResult === 'heads' ? '正面朝上：已立即获得 10 金币，可用余额已更新。' : '反面朝上：本次没有变化。'}</p></div> : cardConfirming.cardId === 'prizeReroll' ? <p>确认后将立刻抽出 6 张新拍品，供你在弹窗中私密选择下一轮拍品。抽取结果会立即锁定，不能取消或重抽。</p> : cardConfirming.cardId === 'prizeSwap' ? <p>确认后将立刻抽出 9 张拍品，供你在弹窗中秘密替换本轮拍品。其他玩家不会收到提示，直到回合结算才会揭晓。</p> : cardTargetScope(cardConfirming.cardId) !== 'none' ? <p>确认后会立刻弹出玩家卡片，请选择这张卡的生效对象；选择前随时可以取消。</p> : <p>确认后，这张卡会安排在本轮结算时使用。</p>}<div>{!(cardConfirming.cardId === 'fateCoin' && coinFlipStarted) && <button className="button button--paper" onClick={() => { setCardConfirming(null); setCoinFlipResult(null); setCoinFlipStarted(false) }}>取消</button>}<button className="button button--primary" disabled={(cardConfirming.cardId === 'sleeveUpgrade' && !upgradeChoices(session, player.id, confirmedCardUses).includes(upgradeTarget!)) || (cardConfirming.cardId === 'luckyTickets' && (!session.lottery || !availableLotteryNumbers(session.lottery, session.players.length).length)) || (cardConfirming.cardId === 'fateCoin' && coinFlipStarted && coinFlipResult === null)} onClick={confirmCardUse}>{cardConfirming.cardId === 'fateCoin' ? !coinFlipStarted ? '确认并掷硬币' : coinFlipResult === null ? '正在掷硬币…' : '知道了' : ['prizeReroll', 'prizeSwap'].includes(cardConfirming.cardId) ? `确认并抽取 ${cardConfirming.cardId === 'prizeSwap' ? 9 : 6} 张` : cardTargetScope(cardConfirming.cardId) !== 'none' ? '确认并选择玩家' : '确认使用'}</button></div></div></div>}
      ...
      {visibleGrant && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="grant-title"><div className="card-grant-sheet"><span className="game-art-slot"><CardIcon id={visibleGrant.cardId} /></span><p className="eyebrow">获得道具 · <CardRarityTag cardId={visibleGrant.cardId} /></p><h2 id="grant-title">{getCardDefinition(visibleGrant.cardId).name}</h2><p>{getCardDefinition(visibleGrant.cardId).description}</p><small>已放入背包。</small><button className="button button--primary" onClick={() => onAcknowledgeGrant(player.id)}>收下</button></div></div>}
...
function RevealReady({ session, onReveal }: { session: GameSession; onReveal: () => void }) {
  return (
    <section className="reveal-ready screen-center">
      <div className="ready-stack">{session.players.slice(0, 5).map((player, index) => <span key={player.id} style={{ '--stack-index': index, '--player-color': player.color } as React.CSSProperties}><PlayerAvatar player={player} /></span>)}</div>
...
function AuctionIntro({ session, onContinue }: { session: GameSession; onContinue: () => void }) {
  ...
  return <section className="round-intro screen-center"><p className="eyebrow">{systemAuction ? '系统发起首轮竞购' : '道具商人发起竞购'}</p><h1>一张公开道具，<br /><em>秘密报价。</em></h1><div className="auction-card"><span className="game-art-slot"><CardIcon id={card.id} /></span><strong>{card.name} <CardRarityTag cardId={card.id} /></strong><small>{card.description}</small></div><p className="lead">{systemAuction ? '所有人依次秘密报价。最高唯一正报价者获得道具，报价支付给系统。' : '所有人都会轮到报价；发起者也会经过流程，但只能报 0。最高唯一正报价者获得道具，报价款归发起者。'}</p><button className="button button--primary button--large" onClick={onContinue}>开始秘密竞购</button></section>
}
...
function SpectatorRoundTakeoverPicker({ players, onClose, onConfirm }: { players: Player[]; onClose: () => void; onConfirm: (playerIds: string[]) => void }) {
  const [selectedIds, setSelectedIds] = useState<string[]>([])
  const togglePlayer = (playerId: string) => setSelectedIds((ids) => ids.includes(playerId) ? ids.filter((id) => id !== playerId) : [...ids, playerId])
  const allSelected = selectedIds.length === players.length
  return <div className="modal-backdrop spectator-takeover-picker" role="dialog" aria-modal="true" aria-labelledby="spectator-takeover-title">
    <section className="target-picker-sheet">
      <p className="eyebrow">下一轮接管</p>
    ...
    <section className={cx('results-page', `reveal-stage-${revealStage}`, skipMotion && 'skip-motion')} data-reveal-stage={revealStage}>
      ...
      {prizeSwapEffect && prizeSwapRevealOpen && <div className="modal-backdrop prize-swap-reveal" role="dialog" aria-modal="true" aria-labelledby="prize-swap-reveal-title">
        ...
        <div className="prize-swap-reveal__frame">
          <span className="prize-swap-reveal__jester game-art-slot" aria-hidden="true"><CardIcon id="prizeSwap" /></span>
      ...
      {showSettlement && result.passivityFeePlayerCount > 0 && <aside className="round-bulletin round-bulletin--warning" aria-live="polite"><span>◌</span><div><small>观望惩罚</small><strong>本轮有 {result.passivityFeePlayerCount} 人受到了观望惩罚。</strong></div></aside>}
      {showSettlement && result.cardEffects.length > 0 && <article className="panel card-effects"><div className="panel-title"><div><p className="eyebrow">结算影响</p><h2>本轮道具与排名变化</h2></div><span>已计入本轮结果</span></div><div>{result.cardEffects.map((effect, index) => <p key={`${effect.cardId ?? effect.symbol}-${index}`} data-effect={effect.cardId ?? 'general'} style={{ '--effect-delay': `${index * 110}ms` } as React.CSSProperties}><span className={cx(effect.cardId && "game-art-slot")}>{effect.cardId ? <CardIcon id={effect.cardId} /> : effect.symbol}</span>{effect.description}</p>)}</div></article>}
      {showSettlement && result.assetAuctionResults.length > 0 && <article className="panel card-effects"><div className="panel-title"><div><p className="eyebrow">拍品竞购</p><h2>藏品竞购结果</h2></div><span>成交价仅买家可见</span></div><div>{result.assetAuctionResults.map((entry) => <p key={entry.lotId}><span>{entry.item.emoji}</span>{entry.winnerId ? <>{entry.item.name} 由 <strong>{playerName(session.players, entry.winnerId)}</strong> 拍下。</> : <>{entry.item.name} 无人拍下，已回归 <strong>{playerName(session.players, entry.sellerId)}</strong>。</>}</p>)}</div></article>}
      <div className="result-columns">
        ...
        {showSettlement && <article className="panel prediction-result"><div className="panel-title"><div><p className="eyebrow">眼光如何</p><h2>预测结算</h2></div>{correctPredictionRewardUnits > 0 && <span>猜中者共获得 {formatCoins(correctPredictionRewardUnits)}</span>}</div>
...
function identityActionReview(action: IdentityAction, players: Player[], divinations: ProphetDivination[] = [], nightwalkerOutcome?: RoundResult['nightwalkerOutcomes'][number]): string {
  if (action.type === 'prophetDivination') {
    ...
    if (divination.mode === 'identity') {
      const guesses = divination.identityGuesses ?? (divination.identityGuess ? [divination.identityGuess] : [])
      return `预言家观身份：${guesses.map((guess) => `${playerName(players, guess.targetPlayerId)}＝${getIdentityDefinition(guess.identityId).name}（${guess.correct ? '猜中' : '猜错'}）`).join('；')}`
    }
  ...
  if (action.type === 'nightwalkerDoubleBid') {
    if (!nightwalkerOutcome) return '夜行者设下了两档暗标'
    const chosen = nightwalkerOutcome.chosenBidUnits === nightwalkerOutcome.shadowBidUnits ? '采用影价 B' : '保留明面 A'
    const reason = nightwalkerOutcome.reason === 'shadowWinsItem' ? '已开启优先拿藏品，只有 B 能获得拍品'
      : nightwalkerOutcome.reason === 'baseWinsItem' ? '已开启优先拿藏品，只有 A 能获得拍品'
...
function RelayTenureReview({ session }: { session: GameSession }) {
  ...
  return <section className="relay-tenure-review panel"><div className="panel-title"><div><p className="eyebrow">接力回顾</p><h2>操作者任期</h2></div><span>{session.relayMethod === 'rotation' ? '按回合轮换' : '分段接力'}</span></div><p className="round-review-note">数据归属于操作者负责的回合；资产仍由同一位游戏玩家继承。</p><div className="relay-tenure-grid">{session.players.flatMap((player) => (player.relayOperators ?? []).map((operator) => {
    ...
    return <article key={`${player.id}-${operator.id}`}><header><span style={{ background: player.color }}><PlayerAvatar player={{ id: operator.id, memberId: operator.memberId, color: player.color }} /></span><div><strong>{operator.name}</strong><small>替 {player.name} · {assigned.length} 回合</small></div></header>{assigned.length === 0 ? <p>本局未轮到这位操作者。</p> : <><div className="relay-tenure-metrics"><span><b>{formatCoins(bidUnits)}</b>下注</span><span><b>{awards}</b>获奖</span><span><b>{items}</b>拍品</span><span><b>{formatCoins(after - before)}</b>资产变化</span></div><small>第 {firstRound! + 1}–{lastRound! + 1} 轮 · 技能/道具 {activeTurns} · 市场获得 道具 {cardWins} / 拍品 {assetWins}</small></>}</article>
  }))}</div></section>
}
        ...
        {session.results.map((result) => {
          ...
          return <details key={result.roundIndex} open={result.roundIndex === session.results.length - 1}>
            ...
            <div className="round-review-grid">
              ...
              {result.lottery && <article className="review-block review-block--wide"><h3>彩票</h3><p>{lotterySummary(result.lottery, session.players)}</p>{result.lottery.tickets.map(ticket => <div className="review-row" key={ticket.number}><strong>{playerName(session.players, ticket.playerId)} · {String(ticket.number).padStart(2, '0')} 号</strong><span>第 {ticket.roundIndex + 1} 轮{ticket.source === 'gift' ? '获赠' : '购入'} · 支付 {formatCoins(ticket.paidUnits)} · 补贴 {formatCoins(ticket.subsidyUnits)}</span></div>)}</article>}
...
function SubLeaderboards({ session }: { session: GameSession }) {
  const standings = rankFinalPlayers(session.players)
  const makeBoard = (id: string, title: string, subtitle: string, values: Record<string, number>) => {
    const rows = [...session.players].sort((left, right) => (values[right.id] ?? 0) - (values[left.id] ?? 0))
    const groups: Player[][] = []
    for (const row of rows) { const prior = groups.at(-1); if (prior && (values[prior[0].id] ?? 0) === (values[row.id] ?? 0)) prior.push(row); else groups.push([row]) }
    return <details className="sub-leaderboard" key={id}><summary><span>{id}</span><div><strong>{title}</strong><small>{subtitle}</small></div><i>展开</i></summary><ol>{groups.map((group, index) => <li key={group[0].id}><b>{index + 1}</b><div>{group.map((player) => <span key={player.id} style={{ '--player-color': player.color } as React.CSSProperties}><i />{player.name}<em>{formatCoins(values[player.id] ?? 0)}</em></span>)}</div></li>)}</ol></details>
  }
  const totalBids = Object.fromEntries(session.players.map((player) => [player.id, session.results.reduce((total, result) => total + (result.turns.find((turn) => turn.playerId === player.id)?.bidUnits ?? 0) + result.investments.filter((investment) => investment.investorId === player.id).reduce((sum, investment) => sum + investment.investmentUnits, 0), 0)]))
  const totalEarnings = Object.fromEntries(standings.map((standing) => [standing.player.id, standing.totalAssetUnits - session.settings.initialCoins * 2]))
  const predictionEarnings = Object.fromEntries(session.players.map((player) => [player.id, session.results.reduce((total, result) => total + (result.deltas.find((delta) => delta.playerId === player.id)?.predictionUnits ?? 0), 0)]))
  return <section className="sub-leaderboards panel"><div className="panel-title"><div><p className="eyebrow">赛后数据</p><h2>数据子排行榜</h2></div><span>点击展开</span></div><div>{makeBoard('①', '总下注额榜', '普通下注 + 跟投金额', totalBids)}{makeBoard('②', '总收益额榜', '终局总资产 − 初始金币（含固定资产）', totalEarnings)}{makeBoard('③', '总预测收益榜', '预测转账收入 − 预测罚款', predictionEarnings)}</div></section>
}
...
function FinalResult({ session, onNewGame, onRematch, onRevenge }: { session: GameSession; onNewGame: () => void; onRematch: () => void; onRevenge: () => void }) {
  ...
  const standingCard = (standing: ReturnType<typeof rankFinalPlayers>[number], index: number) => <article key={standing.player.id} className={cx(standing.place === 1 && 'is-first')} style={{ '--delay': `${index * 90}ms`, '--player-color': standing.player.color } as React.CSSProperties}><span className="standing-place">{standing.place}</span><div className="standing-avatar"><PlayerAvatar player={standing.player} /></div><div className="standing-copy"><strong>{standing.player.name}</strong><small>{standing.player.items.length > 0 ? standing.player.items.map(({ item }) => `${item.emoji}${item.name}`).join(' · ') : '没有收藏品'}</small>{standing.fixedAssets.some((asset) => asset.units > 0) && <div className="asset-breakdown">{standing.fixedAssets.filter((asset) => asset.units > 0).map((asset) => <span key={asset.category}>{ASSET_CATEGORY_CONFIGS.find((entry) => entry.category === asset.category)?.symbol} {categoryConfig(asset.category).name} {asset.itemCount} 件 +{formatCoins(asset.units)}</span>)}</div>}</div><div className="standing-balance"><CoinValue units={standing.totalAssetUnits} /><small>总资产</small><span>现金 {formatCoins(standing.cashUnits)} · 固定资产 +{formatCoins(standing.fixedAssetUnits)}</span></div></article>
  ...
  return (
    <section className="final-page">
      ...
      {fullyRevealed && <section className="champion-podium" aria-label="冠亚季军领奖台"><p className="eyebrow">荣耀时刻</p><h2>前三名登上领奖台</h2><div>{medalists.map((standing, index) => <article key={standing.player.id} className={cx(`champion-podium__place--${3 - index}`, standing.place === 1 && 'is-champion')} style={{ '--delay': `${index * 420}ms`, '--player-color': standing.player.color } as React.CSSProperties}><span>{index === 0 ? '季军' : index === 1 ? '亚军' : '冠军'}</span><div><PlayerAvatar player={standing.player} /></div><strong>{standing.player.name}</strong><small>第 {standing.place} 名 · {formatCoins(standing.totalAssetUnits)}</small></article>)}</div></section>}
      ...
      {fullyRevealed && session.settings.identitySettings.enabled && <section className="identity-final panel"><div className="panel-title"><div><p className="eyebrow">终局揭示</p><h2>身份公开</h2></div><span>所有身份与开局配置</span></div><div>{session.players.map((player) => { const identity = player.identity ? getIdentityDefinition(player.identity.id) : null; const config = player.identity?.id === 'thief' && player.identity.targetPlayerId ? `目标：${playerName(session.players, player.identity.targetPlayerId)}` : player.identity?.collectorCategory ? `收藏类别：${categoryConfig(player.identity.collectorCategory).name}` : ''; return <article key={player.id}><span className="game-art-slot">{identity ? <IdentityIcon id={identity.id} /> : '—'}</span><div><strong>{player.name} · {identity?.name ?? '未选择身份'}</strong><small>{identity?.summary ?? '本局未启用身份。'}{config ? ` ${config}` : ''}</small></div></article> })}</div></section>}
...
function KidnapNegotiationPanel({ negotiation, onResolve }: { negotiation: KidnapNegotiation; onResolve: (payRansom: boolean) => void }) {
  ...
  return <section className="kidnap-negotiation" role="dialog" aria-modal="true" aria-labelledby="kidnap-negotiation-title">
    ...
    <div className="kidnap-negotiation__frame">
      <div className="kidnap-negotiation__threat-mark" aria-hidden="true"><span className="game-art-slot"><IdentityIcon id="assassin" /></span><i>⛓</i></div>
...
function Game({ session, setSession, onExit, onNewGame, onRematch, onRevenge }: { session: GameSession; setSession: (session: GameSession) => void; onExit: () => void; onNewGame: () => void; onRematch: () => void; onRevenge: () => void }) {
  ...
  const [botSpeed, setBotSpeed] = useState(() => { const value = Number(localStorage.getItem('auction-battle:spectator-speed')); return [0.5, 1, 2, 4].includes(value) ? value : 1 })
  const [autoPausedRound, setAutoPausedRound] = useState<number | null>(null)
  const [spectatorPanelTab, setSpectatorPanelTab] = useState<SpectatorPanelTab>(() => (localStorage.getItem('auction-battle:spectator-tab') as SpectatorPanelTab) || 'battle')
  ...
  const [spectatorEventFilter, setSpectatorEventFilter] = useState('all')
  ...
  const instantActionLock = useRef('')
  const resolveInstantCard = (cardId: 'luckyTickets' | 'sleeveUpgrade', target?: CardId, planned: CardUse[] = []) => {
    if (session.operationDeadlineAt && Date.now() >= session.operationDeadlineAt) return false
    const actionKey = `${session.id}:${session.roundIndex}:${session.currentTurnIndex}:${session.instantCardUses?.length ?? 0}`
    if (instantActionLock.current === actionKey) return false
    const changes = useInstantCard(session, session.players[session.currentTurnIndex].id, cardId, target, planned)
    if (!changes) return false
    instantActionLock.current = actionKey
  ...
  const purchaseLottery = (number: number | null, reservedUnits = 0) => {
    const player = session.players[session.currentTurnIndex]
    const key = `${session.id}:${session.roundIndex}:${player?.id}:${session.lottery?.tickets.filter(ticket => ticket.playerId === player?.id && ticket.roundIndex === session.roundIndex && ticket.source !== 'gift').length ?? 0}`
  ...
  const openMerchantShop = (playerId: string): boolean => {
    const player = session.players[session.currentTurnIndex]
    const hasMerchantOffer = session.pendingMerchantOffers.some((offer) => offer.playerId === playerId && offer.roundIndex === session.roundIndex)
    if (session.phase !== 'privateTurn' || player?.id !== playerId || player.identity?.id !== 'merchant' || hasMerchantOffer || player.balanceUnits < MERCHANT_SHOP_ENTRY_FEE_UNITS || session.merchantShops.some((shop) => shop.playerId === playerId && shop.roundIndex === session.roundIndex)) return false
    const offeredCardIds = Array.from(new Set(session.cardDeck)).filter((cardId) => !session.settings.disabledCardIds.includes(cardId)).slice(0, 3)
    if (offeredCardIds.length !== 3) return false
    const cardDeck = [...session.cardDeck]
    for (const cardId of offeredCardIds) cardDeck.splice(cardDeck.indexOf(cardId), 1)
    const cards = offeredCardIds.map((cardId) => ({ cardId, priceUnits: merchantShopPriceUnits(cardId) }))
    patch({
      players: session.players.map((entry) => entry.id === playerId ? { ...entry, balanceUnits: entry.balanceUnits - MERCHANT_SHOP_ENTRY_FEE_UNITS } : entry),
      cardDeck,
      merchantShops: [...session.merchantShops, { playerId, roundIndex: session.roundIndex, cards }],
    })
    return true
  }
  const buyMerchantShopCard = (playerId: string, cardId: CardId): boolean => {
    const player = session.players[session.currentTurnIndex]
    const shop = session.merchantShops.find((entry) => entry.playerId === playerId && entry.roundIndex === session.roundIndex)
    const listing = shop?.cards.find((entry) => entry.cardId === cardId)
    if (session.phase !== 'privateTurn' || player?.id !== playerId || !shop || !listing || player.balanceUnits < listing.priceUnits) return false
    const spectatorPatch = appendSpectatorEvent(session, { roundIndex: session.roundIndex, type: 'auctionResult', playerId, summary: `${player.name} 从道具商店购买 ${getCardDefinition(cardId).name}`, details: [`支付 ${formatCoins(listing.priceUnits)} 金币`] })
    patch({
      players: session.players.map((entry) => entry.id === playerId ? { ...entry, balanceUnits: entry.balanceUnits - listing.priceUnits, cardInventory: [...entry.cardInventory, cardId] } : entry),
      merchantShops: session.merchantShops.map((entry) => entry === shop ? { ...entry, cards: entry.cards.filter((card) => card !== listing) } : entry),
      ...spectatorPatch,
    })
    return true
  }
  const confirmIdentity = (config: { targetPlayerId?: string; collectorCategory?: AssetCategory; merchantCardId?: CardId }, botRecord?: { mode: import('./game/types').StrategyMode; reason: string }) => {
    ...
    if (nextIndex >= players.length) {
      let deck = cardDeck
      const merchantAwards = players.filter((player) => player.identity?.id === 'merchant').flatMap((merchant) => {
        const draw = drawCard(deck, session.settings.disabledCardIds)
        deck = draw.cardDeck
        return draw.cardId ? [{ playerId: merchant.id, cardId: draw.cardId }] : []
      })
      const allAwards = [...pendingAwards, ...merchantAwards]
      const routed = routeCardAwards({ players, awards: allAwards, settings: session.settings.identitySettings, fairnessOrderIds: session.fairnessOrderIds, roundIndex: 0 })
      const delivered = new Set(routed.delivered.map((award) => `${award.playerId}-${award.cardId}`))
      const enabled = enabledIdentityIds(session.settings.identitySettings)
      const prophetIdentityCandidates = routed.players.filter((entry) => entry.identity?.id === 'prophet').reduce<Record<string, Record<string, IdentityId[]>>>((all, prophet) => ({
        ...all,
        [prophet.id]: routed.players.filter((target) => target.id !== prophet.id).reduce<Record<string, IdentityId[]>>((targets, target) => {
          const truth = target.identity?.id
          const alternatives = shuffle(enabled.filter((identityId) => identityId !== truth)).slice(0, 5)
          return truth ? { ...targets, [target.id]: shuffle([truth, ...alternatives]) } : targets
        }, {}),
      }), {})
      patch({ players: routed.players, roundStartBalanceUnits: Object.fromEntries(routed.players.map((player) => [player.id, player.balanceUnits])), cardDeck: deck, pendingIdentityCardAwards: [], pendingCardGrants: allAwards.filter((award) => delivered.has(`${award.playerId}-${award.cardId}`)).map((award) => ({ ...award, announced: false })), pendingIdentityNotices: [...session.pendingIdentityNotices, ...routed.notices], identityEvents: [...session.identityEvents, ...routed.events], identityAvailableIds: enabled, identityDraft: null, prophetIdentityCandidates, phase: 'roundIntro', spectatorTakeoverPlayerIds: [], spectatorTakeoverRoundIndex: null, ...spectatorPatch })
      return
    }
    patch({ players, cardDeck, pendingIdentityCardAwards: pendingAwards, identityAvailableIds: enabledIdentityIds(session.settings.identitySettings), identityDraft: { playerIndex: nextIndex, choiceIds: dealIdentityChoices(selectedIds, session.settings.identitySettings) }, phase: 'identityHandoff', spectatorTakeoverPlayerIds: [], spectatorTakeoverRoundIndex: null, ...spectatorPatch })
  }
  ...
  const useProphetDivination = (playerId: string, mode: ProphetDivination['mode'], targetPlayerId?: string, identityId?: IdentityId): boolean => {
    const player = session.players[session.currentTurnIndex]
    const costUnits = 0
    const ownDivinations = session.prophetDivinations.filter((entry) => entry.playerId === playerId)
    const alreadyUsedMainThisRound = ownDivinations.some((entry) => entry.roundIndex === session.roundIndex && entry.mode !== 'identity')
    const identityDivinationThisRound = ownDivinations.find((entry) => entry.roundIndex === session.roundIndex && entry.mode === 'identity')
    const identityGuessesThisRound = identityDivinationThisRound ? (identityDivinationThisRound.identityGuesses ?? (identityDivinationThisRound.identityGuess ? [identityDivinationThisRound.identityGuess] : [])) : []
    const identityGuessesLeftThisRound = prophetIdentityGuessesRemaining(identityGuessesThisRound)
    const mainUses = ownDivinations.filter((entry) => entry.mode !== 'identity').length
    if (session.phase !== 'privateTurn' || player?.id !== playerId || player.identity?.id !== 'prophet' || (mode === 'identity' ? identityGuessesLeftThisRound <= 0 : alreadyUsedMainThisRound) || (mode !== 'identity' && mainUses >= session.settings.identitySettings.prophetDivinationLimit)) return false
    ...
    if (mode === 'identity') {
      ...
      const candidates = targetPlayerId ? session.prophetIdentityCandidates[playerId]?.[targetPlayerId] : undefined
      const progress = targetPlayerId ? getProphetIdentityProgress(session.prophetDivinations, playerId, targetPlayerId, session.prophetIdentityProgress[playerId]?.[targetPlayerId]) : undefined
      const validIdentity = identityId && (candidates?.includes(identityId) ?? !session.settings.identitySettings.disabledIdentityIds.includes(identityId))
      if (!target || target.id === playerId || !validIdentity || progress?.solvedIdentityId || progress?.excludedIdentityIds.includes(identityId) || !canMakeIdentityGuess(session.prophetDivinations, playerId, target.id, identityId)) return false
      ...
      const guess = { targetPlayerId: target.id, identityId, correct, ...(rewardCardId ? { rewardCardId } : {}) }
      divination = identityDivinationThisRound
        ? { ...identityDivinationThisRound, identityGuesses: [...(identityDivinationThisRound.identityGuesses ?? (identityDivinationThisRound.identityGuess ? [identityDivinationThisRound.identityGuess] : [])), guess] }
        : { id, playerId, roundIndex: session.roundIndex, mode: 'identity', costUnits, identityGuess: guess, identityGuesses: [guess] }
    }
    ...
    const identityFeedbackGuess = divination.identityGuesses?.at(-1) ?? divination.identityGuess
    const notices = mode === 'identity' && identityFeedbackGuess ? [...session.pendingIdentityNotices, {
      id: `prophet-identity-feedback-${divination.id}`,
      playerId,
      title: identityFeedbackGuess.correct ? '观身份猜对了' : '观身份猜错了',
      detail: identityFeedbackGuess.correct
        ? `你猜对了 ${playerName(session.players, identityFeedbackGuess.targetPlayerId)} 的身份：${getIdentityDefinition(identityFeedbackGuess.identityId).name}。${identityFeedbackGuess.rewardCardId ? `获得了 ${getCardDefinition(identityFeedbackGuess.rewardCardId).name}。` : '但循环卡池暂时没有可发道具。'}`
        : '这次身份猜测不正确；该错误身份已被排除，不会有额外罚款。',
    }] : session.pendingIdentityNotices
    ...
    const prophet = session.players.find((entry) => entry.id === playerId)
    const guessDetails = divination.identityGuesses?.map((guess) => `${playerName(session.players, guess.targetPlayerId)}：${getIdentityDefinition(guess.identityId).name} · ${guess.correct ? '猜中' : '猜错'}`) ?? []
    const spectatorPatch = appendSpectatorEvent(session, { roundIndex: session.roundIndex, type: 'turn', playerId, summary: `${prophet?.name ?? '预言家'} 发动${prophetModeLabel(mode)}`, details: mode === 'wealth' && divination.wealth ? [`最高余额约 ${formatCoins(divination.wealth.highestRangeUnits[0])}–${formatCoins(divination.wealth.highestRangeUnits[1])}`, `最低余额约 ${formatCoins(divination.wealth.lowestRangeUnits[0])}–${formatCoins(divination.wealth.lowestRangeUnits[1])}`] : mode === 'stars' ? [`查看未来 ${divination.starItemIds?.length ?? 0} 件拍品`] : guessDetails })
    patch({ players, cardDeck, prophetDivinations: mode === 'identity' && identityDivinationThisRound ? session.prophetDivinations.map((entry) => entry.id === identityDivinationThisRound.id ? divination as ProphetDivination : entry) : [...session.prophetDivinations, divination], prophetIdentityProgress: identityProgress, pendingIdentityNotices: resolvedNotices, pendingProphetCardOffers, ...spectatorPatch })
    return true
  }
  ...
  const resolveFateCoin = (playerId: string, result: 'heads' | 'tails'): CardUse | null => {
    const player = session.players[session.currentTurnIndex]
    if (session.phase !== 'privateTurn' || player?.id !== playerId || !player.cardInventory.includes('fateCoin')) return null
    ...
    patch({
      ...
      pendingFateCoinUse: { playerId, roundIndex: session.roundIndex, use, previousUses: session.pendingFateCoinUse?.playerId === playerId && session.pendingFateCoinUse.roundIndex === session.roundIndex ? [...(session.pendingFateCoinUse.previousUses ?? []), session.pendingFateCoinUse.use] : [] },
    })
    return use
  }
  const submitTurn = (turn: RoundTurn, botRecord?: { mode: import('./game/types').StrategyMode; reason: string; intel?: string }, timedOut = false) => {
    ...
    const submittedFateCoins = turnCardUses(turn).filter((use) => use.cardId === 'fateCoin')
    const lockedImmediate = instantUses(session, turn.playerId)
    const suppliedImmediate = turnCardUses(turn).filter(use => ['luckyTickets', 'sleeveUpgrade'].includes(use.cardId))
    if (suppliedImmediate.some(use => !lockedImmediate.some(locked => JSON.stringify(locked) === JSON.stringify(use)))) return false
    turn = { ...turn, cardUses: [...turnCardUses(turn).filter(use => !['luckyTickets', 'sleeveUpgrade'].includes(use.cardId)), ...lockedImmediate] }
    const pendingFateCoin = session.pendingFateCoinUse?.playerId === turn.playerId && session.pendingFateCoinUse.roundIndex === session.roundIndex ? session.pendingFateCoinUse : null
    if (pendingFateCoin) {
      const locked = [...(pendingFateCoin.previousUses ?? []), pendingFateCoin.use]
      if (locked.length !== submittedFateCoins.length || locked.some((use, index) => use.coinResult !== submittedFateCoins[index].coinResult || use.fateDeltaUnits !== submittedFateCoins[index].fateDeltaUnits)) return false
    } else {
      const fatePlayer = playersBeforeSubmit.find((player) => player.id === turn.playerId)
      for (const use of submittedFateCoins) {
        if (!fatePlayer || !isBot(scheduledActor) || !fatePlayer.cardInventory.includes('fateCoin') || (use.coinResult !== 'heads' && use.coinResult !== 'tails')) return false
        fatePlayer.balanceUnits += use.coinResult === 'heads' ? 20 : 0
        fatePlayer.cardInventory = removeOneCard(fatePlayer.cardInventory, 'fateCoin')
      }
      turn = { ...turn, cardUses: turnCardUses(turn).map((use) => use.cardId === 'fateCoin' ? { ...use, fateDeltaUnits: use.coinResult === 'heads' ? 20 : 0 } : use) }
    }
    const currentPlayer = playersBeforeSubmit.find((player) => player.id === turn.playerId)
    if (!currentPlayer || turn.bidUnits < 0 || turn.bidUnits > currentPlayer.balanceUnits) return false
    const auctionBids = turn.auctionBids ?? []
    const allAuctionLots = [...(session.roundAuctions ?? []), ...(session.roundAssetAuctions ?? [])]
    if (auctionBids.length !== allAuctionLots.length || new Set(auctionBids.map((bid) => bid.lotId)).size !== auctionBids.length) return false
    const auctionBidTotal = auctionBids.reduce((total, bid) => total + bid.bidUnits, 0)
    if (auctionBids.some((bid) => {
      const cardLot = session.roundAuctions.find((lot) => lot.id === bid.lotId)
      const assetLot = session.roundAssetAuctions.find((lot) => lot.id === bid.lotId)
      return !Number.isInteger(bid.bidUnits) || bid.bidUnits < 0 || (!cardLot && !assetLot)
        || (cardLot?.merchantId === turn.playerId && bid.bidUnits !== 0)
        || (assetLot?.sellerId === turn.playerId && bid.bidUnits !== 0)
    ...
    if (turn.identityAction?.type === 'reverserInvert') {
      ...
      const costUnits = currentPlayer.identity?.reverserFreeRoundIndex === session.roundIndex ? 0 : Math.round(session.settings.identitySettings.reverserActivationCoins * (isLastTwoRounds ? 4 : 2))
      if (currentPlayer.identity?.id !== 'reverser' || (currentPlayer.identity.activeSkillUses ?? 0) >= session.settings.identitySettings.reverserActivationLimit || turn.bidUnits + auctionBidTotal + costUnits > currentPlayer.balanceUnits) return false
    }
    ...
    if (turn.identityAction?.type === 'thiefSteal') {
      ...
      if (currentPlayer.identity?.id !== 'thief' || session.roundIndex >= session.settings.rounds - 1 || turn.bidUnits + auctionBidTotal + costUnits > currentPlayer.balanceUnits) return false
    }
    ...
    if (turn.identityAction?.type === 'nightwalkerDoubleBid') {
      ...
      const hasRankChangingCard = turnCardUses(turn).some((use) => ['doubleBid', 'swap', 'bananaPeel', 'reverseRank'].includes(use.cardId))
      if (currentPlayer.identity?.id !== 'nightwalker' || (currentPlayer.identity.nightwalkerUses ?? 0) >= session.settings.identitySettings.nightwalkerUseLimit || (prioritizeItem !== undefined && typeof prioritizeItem !== 'boolean') || !Number.isInteger(shadowBidUnits) || shadowBidUnits <= turn.bidUnits || shadowBidUnits > currentPlayer.balanceUnits || hasRankChangingCard) return false
    }
    const pendingPrizeChanges = session.pendingPrizeChanges.filter((change) => change.playerId === turn.playerId && change.roundIndex === session.roundIndex)
    const lockedPrizeChanges = pendingPrizeChanges.map((change) => !change.confirmedItemId && timedOut
    ...
    const cardUses: CardUse[] = [...turnCardUses(turn).filter((use) => !lockedPrizeCardIds.has(use.cardId)), ...lockedPrizeChanges.map((change) => ({
      ...
      prizeReroll: {
        ...
        offeredItemIds: change.offeredItems.map((item) => item.id),
        chosenItemId: change.confirmedItemId as string,
        targetRoundIndex: change.targetRoundIndex,
      },
    }))]
    if (!validCardMultiplicity(cardUses.map((use) => use.cardId))) return false
    let remainingInventory = [...currentPlayer.cardInventory]
    for (const use of cardUses) {
      if (use.cardId === 'reflectShield' || use.cardId === 'tieCharm') return false
      ...
      if (!isLockedPrizeReroll && !isLockedFateCoin && !lockedImmediate.some(locked => locked.cardId === use.cardId)) {
        if (!remainingInventory.includes(use.cardId)) return false
        remainingInventory = removeOneCard(remainingInventory, use.cardId)
      }
    ...
    let players = playersBeforeSubmit.map((player) => {
      ...
      const updated = {
        ...
        cardInventory: remainingInventory,
      }
    ...
    let auctionQueue = [...session.auctionQueue]
    let cardDeck = [...session.cardDeck]
    const currentMerchantShop = session.merchantShops.find((shop) => shop.playerId === turn.playerId && shop.roundIndex === session.roundIndex)
    const merchantShops = session.merchantShops.filter((shop) => shop !== currentMerchantShop)
    if (currentMerchantShop?.cards.length) cardDeck = shuffle([...cardDeck, ...currentMerchantShop.cards.map((entry) => entry.cardId)])
    if (turn.identityAction?.type === 'merchantAuction') {
      ...
      if (currentPlayer.identity?.id !== 'merchant' || merchantCount >= session.settings.identitySettings.merchantAuctionLimit || currentPlayer.identity.merchantLastAuctionRound === session.roundIndex || session.roundIndex >= session.settings.rounds - 2) return false
      const pendingOffer = session.pendingMerchantOffers.find((offer) => offer.playerId === turn.playerId && offer.roundIndex === session.roundIndex && offer.chosenCardId)
      if (!pendingOffer?.chosenCardId) return false
      ...
      const scheduled = { source: 'merchant' as const, merchantId: turn.playerId, cardId: pendingOffer.chosenCardId, roundIndex: session.roundIndex + 1, bidderIndex: 0, bids: [] }
      if (merchantAuction) auctionQueue = [...auctionQueue, scheduled]
      else merchantAuction = scheduled
      cardDeck = shuffle([...cardDeck, ...pendingOffer.offeredCardIds.filter((cardId) => cardId !== pendingOffer.chosenCardId)])
    }
    if (turn.identityAction?.type === 'lobbyistContract') {
      if (currentPlayer.identity?.id !== 'lobbyist' || (currentPlayer.identity.activeSkillUses ?? 0) >= session.settings.identitySettings.lobbyistActivationLimit || session.roundIndex >= session.settings.rounds - 1 || currentPlayer.identity.lobbyistLastIssuedRound === session.roundIndex) return false
      const action = turn.identityAction
      ...
      actor.identity = { ...identity, lobbyistNextFree: false, lobbyistLastIssuedRound: session.roundIndex, activeSkillUses: (identity.activeSkillUses ?? 0) + 1 }
  ...
  const reveal = () => {
    ...
    const manualTakeoverRound = session.spectatorTakeoverRoundIndex === session.roundIndex && session.spectatorTakeoverPlayerIds.length > 0
    ...
    for (const lot of session.roundAuctions ?? []) {
      ...
      bids.forEach((bid) => counts.set(bid.bidUnits, (counts.get(bid.bidUnits) ?? 0) + 1))
      const winnerBid = bids.filter((bid) => counts.get(bid.bidUnits) === 1).sort((left, right) => right.bidUnits - left.bidUnits)[0]
      if (winnerBid) {
        const winner = auctionPlayers.find((player) => player.id === winnerBid.playerId)
        ...
        if (winner && winner.balanceUnits >= winnerBid.bidUnits) {
          ...
          winner.cardInventory.push(lot.cardId)
          if (merchant) {
    ...
    const assetAuctionResults: AssetAuctionResult[] = []
    for (const lot of session.roundAssetAuctions ?? []) {
      const bids = session.turns.map((turn) => ({ playerId: turn.playerId, bidUnits: turn.auctionBids?.find((bid) => bid.lotId === lot.id)?.bidUnits ?? 0 }))
        .filter((bid) => bid.playerId !== lot.sellerId && bid.bidUnits >= lot.minimumBidUnits)
      const counts = new Map<number, number>()
      bids.forEach((bid) => counts.set(bid.bidUnits, (counts.get(bid.bidUnits) ?? 0) + 1))
      const winnerBid = bids.filter((bid) => counts.get(bid.bidUnits) === 1).sort((left, right) => right.bidUnits - left.bidUnits)[0]
    ...
    const settled = settleRound({
      ...
      disabledCardIds: session.settings.disabledCardIds,
      ...
      roundStartBalanceUnits: session.roundStartBalanceUnits,
    ...
    const marketSpectatorPatch = appendSpectatorEvents(session, spectatorMarketEvents)
  ...
  const resolveKidnapNegotiation = (payRansom: boolean) => {
    ...
    let rewardDeck = negotiation.cardDeck
    if (payRansom) {
      const reward = rewardConnoisseurItem(captured, { item: negotiation.item, roundIndex: session.roundIndex }, session.roundIndex, rewardDeck, session.settings.disabledCardIds)
      rewardDeck = reward.cardDeck
      if (reward.event) {
        result.identityEvents.push(reward.event)
        if (capturedDelta) capturedDelta.identityUnits += reward.event.deltaUnits
      }
    }
  ...
  const beginNormalRound = (roundIndex: number, basePlayers: Player[], baseDeck: CardId[], notices = session.pendingIdentityNotices, events = session.identityEvents, spectatorState: Partial<Pick<GameSession, 'spectatorEvents' | 'pendingSpectatorEvents'>> = {}, takeoverState: Pick<GameSession, 'spectatorTakeoverPlayerIds' | 'spectatorTakeoverRoundIndex'> = { spectatorTakeoverPlayerIds: [], spectatorTakeoverRoundIndex: null }) => {
    ...
    const merchantAwards: Array<{ playerId: string; cardId: CardId }> = []
    const reverserAwards: Array<{ playerId: string; cardId: CardId }> = []
    for (const merchant of grants.players.filter((entry) => entry.identity?.id === 'merchant' && roundIndex < session.settings.rounds - 2)) {
      const draw = drawCard(deck, session.settings.disabledCardIds)
      if (!draw.cardId) continue
      deck = draw.cardDeck
      merchantAwards.push({ playerId: merchant.id, cardId: draw.cardId })
    }
    for (const reverser of grants.players.filter((entry) => entry.identity?.id === 'reverser' && entry.identity.reverserFreeRoundIndex === roundIndex)) {
      const draw = drawCard(deck, session.settings.disabledCardIds)
      if (!draw.cardId) continue
      deck = draw.cardDeck
    ...
    const cardAuctionNotices = roundAuctions.length === 0 ? [] : routed.players.map((player) => ({
      ...
      detail: roundAuctions.map((lot) => {
        const card = getCardDefinition(lot.cardId)
        return `「${card.name}」正在竞购：${card.description}`
      }).join('\n'),
    }))
  ...
  const nextRound = (takeoverPlayerIds: string[] = []) => {
    ...
    if (session.roundIndex + 1 >= session.settings.rounds) {
      ...
      beginNormalRound(nextRoundIndex, session.players, recycledCardDeck, [...session.pendingIdentityNotices, ...taskNotices], session.identityEvents, {}, { spectatorTakeoverPlayerIds: takeoverPlayerIds, spectatorTakeoverRoundIndex: takeoverPlayerIds.length ? nextRoundIndex : null })
    }
  }
  const submitAuctionBid = (bidUnits: number, botRecord?: { mode: import('./game/types').StrategyMode; reason: string }) => {
    ...
    if (auction.bidderIndex < bidders.length - 1) {
      patch({ players: recordedPlayers, merchantAuction: { ...auction, bidderIndex: auction.bidderIndex + 1, bids }, operationDeadlineAt: null, phase: 'auctionHandoff', ...bidSpectatorPatch })
      return
    }
    ...
    const resultSpectatorPatch = appendSpectatorEvent({ ...session, ...bidSpectatorPatch }, { roundIndex: auction.roundIndex, type: 'auctionResult', playerId: winnerBid?.playerId, counterpartyPlayerId: auction.merchantId ?? undefined, bidUnits: winnerBid?.bidUnits, lotId: `${auction.source}-${auction.cardId}`, summary: winnerBid ? `${playerName(players, winnerBid.playerId)} 拍下 ${getCardDefinition(auction.cardId).name}` : `${getCardDefinition(auction.cardId).name} 流拍`, details: [winnerBid ? `成交价 ${formatCoins(winnerBid.bidUnits)} 金币` : '没有唯一正报价'] })
    if (nextAuction) {
      patch({ players, cardDeck: deck, pendingIdentityNotices: notices, identityEvents: events, merchantAuction: nextAuction, auctionQueue: session.auctionQueue.slice(1), operationDeadlineAt: null, phase: 'auctionIntro', ...resultSpectatorPatch })
  ...
  const startSpectatorRoundTakeover = (playerIds: string[]) => {
    if (session.phase !== 'roundResult' || session.roundIndex + 1 >= session.settings.rounds || playerIds.length === 0) return
    setBotPaused(false)
    nextRound(playerIds)
  }
  const acknowledgeSpectatorEvent = () => {
    if (!session.pendingSpectatorEvents.length) return
    patch({ pendingSpectatorEvents: session.pendingSpectatorEvents.slice(1) })
  }
  const stepSpectator = () => {
    if (session.pendingSpectatorEvents.length) acknowledgeSpectatorEvent()
    else { setSpectatorStepPending(true); setBotPaused(false) }
  }
  useEffect(() => {
    if (!spectatorUiActive || !session.pendingSpectatorEvents.length || botPaused) return
    const delay = Math.max(280, 1500 / botSpeed)
    const timer = window.setTimeout(() => patch({ pendingSpectatorEvents: session.pendingSpectatorEvents.slice(1) }), delay)
    return () => window.clearTimeout(timer)
  }, [spectatorUiActive, session.pendingSpectatorEvents, botPaused, botSpeed])
  useEffect(() => {
    if (!spectatorUiActive || !spectatorStepPending || !session.pendingSpectatorEvents.length) return
    setBotPaused(true)
    setSpectatorStepPending(false)
  }, [spectatorUiActive, spectatorStepPending, session.pendingSpectatorEvents.length])
  useEffect(() => {
    if (spectatorUiActive && (botPaused || session.pendingSpectatorEvents.length > 0)) return
    if (spectatorUiActive && session.phase === 'roundResult' && autoPausedRound !== session.roundIndex) {
      setBotPaused(true)
    ...
    const timer = window.setTimeout(() => {
      if (session.phase === 'finalReceiptHandoff' || session.phase === 'finalReceipt') {
        ...
        try {
        if (session.lottery && session.lottery.tickets.filter(ticket => ticket.playerId === currentPlayer.id && ticket.roundIndex === session.roundIndex && ticket.source !== 'gift').length < 2 && availableLotteryNumbers(session.lottery, session.players.length).length && botWantsLottery(session.id, session.roundIndex, currentActor, session.lottery.openingPoolUnits, session.players.length, session.roundIndex === session.settings.rounds - 1)) {
          if (purchaseLottery(null)) return
        }
        if (currentPlayer.cardInventory.includes('luckyTickets') && resolveInstantCard('luckyTickets')) return
        const upgrade = upgradeChoices(session, currentPlayer.id).find(id => id !== 'sleeveUpgrade')
        if (upgrade && resolveInstantCard('sleeveUpgrade', upgrade)) return
        ...
        if (merchantShopWindow && merchantShop?.cards.length) {
          const rarityValue = { common: 1, rare: 1.3, uncommon: 1.55, legendary: 2.15 } as const
          const reserve = session.roundIndex >= session.settings.rounds - 1 ? 0 : 4
          const purchase = [...merchantShop.cards]
            .filter((entry) => entry.priceUnits <= Math.max(0, currentPlayer.balanceUnits - reserve))
            .sort((left, right) => rarityValue[getCardDefinition(right.cardId).rarity] / right.priceUnits - rarityValue[getCardDefinition(left.cardId).rarity] / left.priceUnits || left.priceUnits - right.priceUnits)[0]
          if (purchase && buyMerchantShopCard(currentPlayer.id, purchase.cardId)) return
        }
        ...
        nextRound()
      } else if (spectatorMode && session.phase === 'auctionIntro') {
        patch({ phase: 'auctionHandoff' })
      }
    }, spectatorMode ? Math.max(45, 250 / botSpeed) : 250)
    return () => window.clearTimeout(timer)
  }, [session, botPaused, botSpeed, autoPausedRound, spectatorMode, spectatorUiActive, currentPlayerTakeover])
  ...
  const gameStage = spectatorEvent ? <SpectatorEventStage event={spectatorEvent} session={session} /> : <>
    ...
    {session.phase === 'identityDraft' && (draftActor && isBot(draftActor) ? <BotThinking player={draftActor} operatorName={draftOperator?.name} allBots={spectatorMode} /> : <IdentityDraft key={session.identityDraft?.playerIndex} session={session} onChoose={chooseIdentity} onConfirm={confirmIdentity} />)}
  ...
  return (
    <AppShell quiet={session.phase === 'handoff' || session.phase === 'identityHandoff' || session.phase === 'auctionHandoff'}>
      ...
      {spectatorUiActive && session.phase !== 'finalResult' && <SpectatorControls paused={botPaused} speed={botSpeed} onToggle={() => setBotPaused((value) => !value)} onStep={stepSpectator} onSpeed={setBotSpeed} onPanel={() => setSpectatorPanelOpen(true)} />}
      {spectatorUiActive && session.phase !== 'finalResult' ? <div className="spectator-layout"><main>{gameStage}</main><SpectatorDashboard session={session} tab={spectatorPanelTab} open={spectatorPanelOpen} selectedPlayerId={spectatorPlayerId} eventFilter={spectatorEventFilter} onTab={setSpectatorPanelTab} onClose={() => setSpectatorPanelOpen(false)} onSelectPlayer={setSpectatorPlayerId} onFilter={setSpectatorEventFilter} /></div> : gameStage}
    </AppShell>
  )
}
