...
function RoundResults({ session, result, onNext }: { session: GameSession; result: RoundResult; onNext: () => void }) {
  ...
  return (
    <section className={cx('results-page', `reveal-stage-${revealStage}`, skipMotion && 'skip-motion')} data-reveal-stage={revealStage}>
      ...
      <div className="result-metrics result-metrics--three"><div><small>本轮总下注</small><CoinValue units={result.totalBidUnits} /></div><div><small>并列出局</small><strong>{result.tiedPlayerIds.length} 人</strong></div><div><small>观望费</small><strong>{result.passivityFeePlayerCount} 人</strong></div></div>
      ...
      {showSettlement && result.passivityFeePlayerCount > 0 && <aside className="round-bulletin round-bulletin--warning" aria-live="polite"><span>◌</span><div><small>观望费</small><strong>本轮有 {result.passivityFeePlayerCount} 人需要支付观望费。</strong></div></aside>}
