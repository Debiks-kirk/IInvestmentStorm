...
describe('对局存档迁移', () => {
  ...
  it('旧存档补齐默认操作时限，并保留已经开始的绝对截止时间', () => {
    ...
    values.set('who-is-raising:session:v1', JSON.stringify(legacy))
    expect(loadSession()).toMatchObject({ version: 29, operationDeadlineAt: null, settings: { turnTimeLimitSeconds: 20, turnTimerEnabled: false, systemAuctionCardsPerRound: 1 } })
  })
