...
type ProfileTab = 'overview' | 'matches' | 'data' | 'relationships'
...
export function MemberAvatar({ member, small = false }: { member: MemberProfile; small?: boolean }) {
  const marks = ['◇', '○', '△', '□', '✦', '◒', '⌁', '⬡']
  return <span className={`member-avatar${small ? ' member-avatar--small' : ''}`} style={{ '--member-accent': member.avatar.accent } as React.CSSProperties} aria-hidden="true">{marks[member.avatar.shape % marks.length]}</span>
}
...
export function MemberHall({
  ...
  const pagedMembers = visible.slice(currentPage * pageSize, (currentPage + 1) * pageSize)
  ...
  return <section className="member-hall">
    ...
    <div className="member-grid">
      {pagedMembers.map((member) => {
        ...
        return <button className="member-card" key={member.id} onClick={() => setSelectedId(member.id)} style={{ '--member-accent': member.avatar.accent } as React.CSSProperties}>
          <MemberAvatar member={member} />
          <span className="member-card__copy"><strong>{member.name}</strong><small>{member.kind === 'bot' ? `Bot · ${strategy}` : '真人玩家'}</small></span>
          {stats.matches ? <span className="member-card__record"><b>{winRate(stats.champions, stats.matches)}</b><small>{stats.matches} 局</small></span> : <span className="member-card__record member-card__record--empty">尚未出战</span>}
        </button>
      })}
      {!visible.length && <div className="member-empty"><strong>没有匹配的成员</strong><span>可以注册玩家或创建一位 Bot。</span></div>}
    </div>
    {visible.length > pageSize && <nav className="member-pagination" aria-label="成员分页"><button disabled={currentPage === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}>上一页</button><span>{currentPage + 1} / {pages}</span><button disabled={currentPage + 1 >= pages} onClick={() => setPage((value) => Math.min(pages - 1, value + 1))}>下一页</button></nav>}
...
function MemberProfilePage({ member, members, records, onBack, onChange, onArchive, onRestore, onOpenMatch, onRemoveMatch, onCloneBot }: {
  ...
  return <section className="member-profile">
    ...
    <nav className="member-profile__tabs" aria-label="个人主页分页">{([['overview', '概览'], ['matches', '战绩'], ['data', '数据'], ['relationships', '关系']] as const).map(([value, label]) => <button key={value} className={tab === value ? 'is-active' : ''} onClick={() => setTab(value)}>{label}</button>)}</nav>
    {tab === 'overview' && <div className="member-profile__body member-overview">
      ...
      <section className="member-profile-card"><header><h2>精选成就</h2></header><div className="member-achievements">{featuredAchievements.map((achievement) => <article key={achievement.id}><span>✦</span><div><strong>{achievement.name}</strong><small>{achievement.detail}</small></div></article>)}{!dashboard.achievements.length && <p>完成对局后会在这里留下实绩。</p>}</div></section>
