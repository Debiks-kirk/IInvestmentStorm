...
export function MemberHall({
  ...
  return <section className="member-hall">
    ...
    {showRanking && <details className="rating-rules"><summary>等级分规则</summary><p>首局以 1200 分为基准结算；前 ⌈人数÷3⌉ 名加分，其他名次扣分，并列同分。加分为 60×（加分人数−名次＋1）÷加分人数；扣分为 60×（名次−加分人数）÷其余人数，四舍五入。标准与接力均计入，接力须实际操作过回合。未参赛不排名；移除战绩会撤回该局分数。</p></details>}
