...
- `progression.ts`：`ratingLimit` 通过 (3,6)/(6,13)/(10,20) 分段插值；`ratingDelta` 将正负区间按名次分段、资产偏离平均值 50% 封顶，最终正负对称取整，并列占位平均，全员并列归零。`MatchCareerRecord.finalSeats` 可选保存全部席位资产与共享名次；旧局优先按 seatPlayerId 去重还原，数据不足时取分段中点。`careerRatings` 单次按 session/member 去重并聚合，缺席成员不创建分数。
...
- `LotteryTicket` 记录玩家、购入回合、号码、实付、补贴与操作者成员 ID；`RoundResult.lottery` 保存已锁定开奖和全部有效票根。普通轮全范围均匀抽号，末轮只抽有效号。
- `Game.reveal` 无绑票时直接调用 `settleLottery`；有绑票时在 `resolveKidnapNegotiation` 最后调用。仅更新彩票、现金、领跑者及资产快照，不混入预测/排名/身份收益。
- 彩票状态或结果的 `patch` 在交还 UI 前同步保存，App 常规存档仍保留。`acknowledged` 持久化弹窗关闭；下一轮入口在开奖未确认时拒绝推进。
- `ui/Lottery.tsx` 提供紧凑彩票入口、独立选号/票根弹窗、轮初信息和顶层开奖弹窗；动画不负责抽奖，跳过仅改变表现。号码只公开占用状态，未提交的私人草稿全部计入购票余额检查。
- Bot 用自己的现金、风险参数、轮初池与会话种子确定参与倾向，购票后重新按余款规划回合。观战行动展示已确认票根，混合局不公开。
- v36 职业档案资格仍保留；v36 及更早存档不新增彩票字段，不重算历史结算。

## 保险师、鉴赏家与两张普通卡（2026-09-06）

- 保险师按最终奖区判定，全额返还普通实际下注减香蕉皮已退款后的净损失；免观望惩罚，不返还预测、投资或竞购支出。
- `connoisseur.ts` 统一实际获藏品奖励，`connoisseurCategories` 保存曾获类别，`connoisseurItemKeys` 使用原始 `roundIndex:item.id` 标记实体，不能按当前持有数判断。
- 2026-09-07 更新：金币 5/10/15/20 与道具 1/2/3/4 选一均按首次新类别触发。同类新拍品不再发卡；可选字段 `connoisseurOffers` 在结算时锁定不同候选并暂存实体牌，确认领取后未选牌回池。
- 选卡在下次私密操作前领取；末轮走中立传递与终局奖励入口，刷新不直接跳过。Bot／超时从已锁定候选自动领取，不重新抽牌。启用牌种或池内不同道具不足时按实际可用数量发放，不伪造候选。
- 市场买入先于本轮拍品；绑票未决不预发奖励，赎回后通过同一奖励函数发放。结算返回更新后的实体牌堆；界面、模拟器必须接收 `settled.cardDeck`。
- 凯旋礼金计入 `cardUnits` 而非排名奖金，不参加投资分红；每张先向下取整再累计。失算保单免除本轮猜错罚款，赌徒真实与公开罚款均归零，不影响跳过罚款；多张不重复获益。
- 单件真实拍品资产统一为 `ceil(value / 4)`，套装档位不变；页面、Bot 边际估值及终局均复用 `itemFixedAssetCoins`。收藏家虚拟件不获得单件加成。
...
- 手绘随成员 IndexedDB 档案及现有备份导入导出保存，不修改会话版本或结算规则；`SMOKE_ONLY=avatars` 覆盖真实触摸、笔画像素、保存恢复、备份往返及四尺寸布局。
- `AvatarMembers` 提供当前成员外观，游戏席位以 `memberId` 查找；无绑定的旧对局按局内 ID 稳定生成外观，不按姓名关联。
- `GameIcon.css` 给身份复盘固定 48px 独立列；`AvatarIcon.css` 统一视口与中心位置，避免字体基线和旧样式造成偏移。

## 配置名册与接力布局

- `registerConfigurationPlayers` 对系统及保存配置均调用永久成员注册，再兼容旧姓名列表；选人弹窗与玩家大厅使用同源档案。
- 接力添加按钮位于席位标题栏，保留可访问名称及 44px 触摸区域；手机排班信息取消大框、轮换选择取消重复副标题。
- 浏览器回归检查名册即时更新及刷新恢复、360px 单操作者席位高度、开始按钮命中和横向溢出；随机改拍道具测试完成候选选择后必须确认。
...
- 结算引擎先按贡献比例拆分排名奖励，再仅对投资者的那一份应用倍率，并向其余额追加向下取至半金币单位的结果。目标玩家的奖励份额、预测、拍品归属和固定资产均不受影响。
- `BotObservation` 携带倍率，投资计划只放大预期现金分红，不虚增拍品类别收益。

## Bot 公开藏品经营

- `PublicRoundObservation` 额外记录公开的最终拍品归属与拍品竞购转移。`publicCategoryCounts` 仅用这些公开复盘重建类别持有量，不访问对手库存。
- Bot 卖家对每名对手计算本拍品带来的单品与套装边际固定资产；若将触发高价值断点，`antiGiftFloor` 会抬高保留价，且低于保护价的候选直接拒绝挂牌。买方估值同步提高真实套装跃迁的权重。
...
- v30 为候选状态增加 `confirmedItemId`：`chosenItemId` 只是可改选的界面选择，只有确认后才更新 `itemDeck`。预言家更新日志时仅观身份替换其同回合身份记录；观财/观星始终追加独立记录。

## Bot 的公开局势记忆与拍品定价

- `PublicRoundObservation` 保存公开的 `rankingReversalCount` 与道具影响 ID；Bot 只据此生成“可能仍有逆转/高波动卡”的概率信号，绝不读取对手库存或身份。
- 常规 Bot 是否对这种信号降价/留现由本局稳定人格种子抽样；逆转者还会在部分高压局面保留正常排名计划，避免自身技能形成固定可反制模式。
- 拍品起拍价由自身损失、拍品内在底价、公开对手类别跳档收益和公开现金区间共同决定；濒临破产且缺少翻盘资源时才允许温和折价，仍保持内在价值底线。
