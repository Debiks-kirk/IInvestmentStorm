...
> 当前有效的架构、数据流与关键技术决策。最后更新：2026-08-05（六套系统配置与人数平衡）。
...
- `createDefaultSettings(playerCount)` 是新局与系统预设的共同平衡基线：3 人为 5 轮／30 金币／夜行者 2／商人 1／赌徒命中 +33%；6 人为 8 轮／30 金币／夜行者 2／商人 3／赌徒 +67%、猜错跳过 −40%、绑匪费用 3；10 人为 10 轮／30 金币／夜行者 3／商人 3／赌徒 +100%、猜错跳过 −33%、绑匪费用 2。其他人数继续使用通用默认，再允许从高级设置细调。
...
- `settleRound` 保持 UI 无关：在原有结算后处理赌徒、绑匪和说客；逆转者通过实际奖励与公开奖励双字段隔离秘密差额。收藏家在 `rankFinalPlayers` 注入额外类别件数，并在实际获得同类别拍品后追加固定 5 金币；该奖励写入 `identityUnits` 和私密事件，且在常规预测付款后发放，不写入公共账本。
