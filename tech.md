...
> 当前有效的架构、数据流与关键技术决策。最后更新：2026-08-04（说客任务卡）。
...
- 使用改拍令时，`pendingPrizeReroll` 会立刻持久化使用者、回合、原下一拍品和从 `ITEM_POOL` 中排除全部已安排拍品后抽得的 3 张候选；卡牌同时从私密库存移除并占用本回合两张道具之一。选择后将 `chosenItemId` 锁定并写入实际牌堆，提交回合时才转为 `CardUse`、进入下轮循环回收。
