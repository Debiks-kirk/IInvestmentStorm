...
> 当前有效的架构、数据流与关键技术决策。最后更新：2026-08-05（身份规则与私密反馈）。
...
- 会话格式升为 v9。`IdentitySettings` 具有独立的赌徒猜错／跳过倍率与商人拍卖次数上限；旧存档以旧跳过倍率补齐猜错倍率，旧商人的 `merchantAuctionUsed` 映射为已发动一次。
...
- 旧会话缺少新设置时迁移为 `false`，不改变已进行对局；旧商人竞购缺少 `source` 时按 `merchant` 兼容。新会话使用 v9 格式，加载时会将缺字段立即回写。
