...
> 当前有效的架构、数据流与关键技术决策。最后更新：2026-08-04（Bot 自动推进防卡死）。
...
- 说客 Bot 通过 `BotObservation.lobbyistFeeUnits` 获取本回合真实可支付的随机任务费用，仅在“投资 + 费用”不超过余额时安排任务。`Game` 在自动 Bot 提交被拒绝时立即再提交无下注、无预测、无道具的保底回合；该保底只用于推进状态机，正常计划仍先走原有完整校验。
