...
> 当前有效的架构、数据流与关键技术决策。最后更新：2026-08-05（操作时限与道具确认）。
...
- 当前会话格式为 v10。`IdentitySettings` 具有独立的赌徒猜错／跳过倍率与商人拍卖次数上限；旧存档以旧跳过倍率补齐猜错倍率，旧商人的 `merchantAuctionUsed` 映射为已发动一次。
...
- `GameSettings.turnTimeLimitSeconds` 默认 20，合法范围 5–120 秒；配置预设与迁移都会保存/补齐该设置。`GameSession.operationDeadlineAt` 是绝对时间戳，私密页面刷新虽降级回传递页，仍不会重新计时。
